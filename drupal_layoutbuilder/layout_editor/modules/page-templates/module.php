<?php
namespace DrupalLayoutbuilder\Modules\PageTemplates;
...
use DrupalLayoutbuilder\Core\DocumentTypes\Post as PostDocument;
use DrupalLayoutbuilder\DB;
use DrupalLayoutbuilder\Modules\Library\Documents\Page as PageDocument;
...
class Module extends BaseModule {
	...
	public function template_include( $template ) {
		if ( is_singular_layoutbridge_adapter() ) {
			$document = Plugin::$instance->documents->get_doc_for_frontend( get_the_ID_layoutbridge_adapter() );
	...
	public function add_wp_templates_support() {
		...
		foreach ( $post_types as $post_type ) {
			add_filter_layoutbridge_adapter( "theme_{$post_type}_templates", [ $this, 'add_page_templates' ], 10, 4 );
		}
	}
	...
	public function add_page_templates( $page_templates, $wp_theme, $post ) {
		...
		$page_templates = [
			self::TEMPLATE_CANVAS => _x_layoutbridge_adapter( 'Elementor Canvas', 'Page Template', 'elementor' ),
			self::TEMPLATE_HEADER_FOOTER => _x_layoutbridge_adapter( 'Elementor Full Width', 'Page Template', 'elementor' ),
		] + $page_templates;
	 ...
	 * Print callback.
	 *
	 * Prints the page template content using Drupal loop.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function print_callback() {
		while ( have_posts_layoutbridge_adapter() ) :
			the_post();
			the_content();
		endwhile;
	}
	...
	public function register_template_control( $document, $control_id = 'template' ) {
		...
		$document->add_control(
			$control_id,
			[
				'label' => ___layoutbridge_adapter( 'Page Layout', 'elementor' ),
		...
		$document->add_control(
			...
			[
				...
				'raw' => ___layoutbridge_adapter( 'Default Page Template from your theme', 'elementor' ),
				'separator' => 'none',
		...
		$document->add_control(
			...
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw' => ___layoutbridge_adapter( 'No header, no footer, just Elementor', 'elementor' ),
				'separator' => 'none',
				'content_classes' => 'drupal_layoutbuilder-descriptor',
				'condition' => [
		...
		$document->add_control(
			...
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw' => ___layoutbridge_adapter( 'This template includes the header, full-width content and footer', 'elementor' ),
				'separator' => 'none',
				'content_classes' => 'drupal_layoutbuilder-descriptor',
				'condition' => [
	 ...
	 *
	 * Elementor don't allow Drupal to update the parent page template
	...
	public function filter_update_meta( $check, $object_id, $meta_key ) {
		if ( '_wp_page_template' === $meta_key ) {
			...
			if ( $is_autosave_action && ! wp_is_post_autosave_layoutbridge_adapter( $object_id ) && DB::STATUS_DRAFT !== get_post_status_layoutbridge_adapter( $object_id ) ) {
				$check = false;
			}
		}
	...
	public function __construct() {
		add_action_layoutbridge_adapter( 'init', [ $this, 'add_wp_templates_support' ] );

		add_filter_layoutbridge_adapter( 'template_include', [ $this, 'template_include' ], 11 /* After Plugins/WooCommerce */ );

		add_action_layoutbridge_adapter( 'elementor/documents/register_controls', [ $this, 'action_register_template_control' ] );

		add_filter_layoutbridge_adapter( 'update_post_metadata', [ $this, 'filter_update_meta' ], 10, 3 );
	}
}
