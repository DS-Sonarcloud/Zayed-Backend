<?php
namespace DrupalLayoutbuilder\Core\Responsive;
...
class Responsive {
		...
		return array_reduce(
			array_keys( self::$default_breakpoints ), function( $new_array, $breakpoint_key ) {
				if ( ! in_array( $breakpoint_key, self::$editable_breakpoints_keys ) ) {
					...
					$saved_option = get_option_layoutbridge_adapter( self::BREAKPOINT_OPTION_PREFIX . $breakpoint_key );
	...
	public static function get_stylesheet_templates_path() {
		return DRUPAL_LAYOUTBUILDER_ASSETS_PATH . 'css/templates/';
	}
	...
	private static function get_stylesheet_templates() {
		...
		return apply_filters_layoutbridge_adapter( 'elementor/core/responsive/get_stylesheet_templates', $templates );
	}
}
