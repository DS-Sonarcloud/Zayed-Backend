...
namespace DrupalLayoutbuilder\Core\Responsive\Files;

use DrupalLayoutbuilder\Core\Files\Base;
...
class Frontend extends Base {
	...
	const META_KEY = 'drupal_layoutbuilder-custom-breakpoints-files';
	...
	public function parse_content() {
		...
		$file_content = preg_replace_callback( '/DRUPAL_LAYOUTBUILDER_SCREEN_([A-Z]+)_([A-Z]+)/', function ( $placeholder_data ) use ( $breakpoints_keys, $breakpoints ) {
	...
	protected function update_meta( $meta ) {
		...
		update_option_layoutbridge_adapter( static::META_KEY, $option );
	}
	...
	protected function delete_meta() {
		...
		if ( $option ) {
			...
			delete_option_layoutbridge_adapter( static::META_KEY );
		}
	}
	...
	private function load_meta_option() {
		$option = get_option_layoutbridge_adapter( static::META_KEY );
