...
use DrupalLayoutbuilder\Scheme_Base;
...
class Global_CSS extends Base {
	 ...
	 */
	const FILE_HANDLER_ID = 'drupal_layoutbuilder-global';
	...
	protected function is_update_required() {
		...
		$schemes_last_update = get_option_layoutbridge_adapter( Scheme_Base::LAST_UPDATED_META );

		if ( $file_last_updated < $schemes_last_update ) {
			return true;
		}

		$elementor_settings_last_updated = get_option_layoutbridge_adapter( Settings::UPDATE_TIME_FIELD );
	...
	private function render_schemes_css() {
		...
		foreach ( $drupal_layoutbuilder->widgets_manager->get_widget_types() as $widget ) {
			...
			foreach ( $scheme_controls as $control ) {
				$this->add_control_rules(
					$control, $widget->get_controls(), function( $control ) use ( $elementor ) {
						$scheme_value = $drupal_layoutbuilder->schemes_manager->get_scheme_value( $control['scheme']['type'], $control['scheme']['value'] );
						...
						return $scheme_value;
					}, [ '{{WRAPPER}}' ], [ '.drupal_layoutbuilder-widget-' . $widget->get_name() ]
				);
			}
		}
	}
