...
abstract class Base extends Base_File {
	...
	protected function use_external_file() {
		return 'internal' !== get_option_layoutbridge_adapter( 'elementor_css_print_method' );
	}
	...
	public function enqueue() {
		...
		if ( self::CSS_STATUS_INLINE === $meta['status'] ) {
			...
			wp_enqueue_style_layoutbridge_adapter( $this->get_file_handle_id(), $this->get_url(), $this->get_enqueue_dependencies(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		 ...
		 */
		do_action_layoutbridge_adapter( "elementor/{$name}-css-file/enqueue", $this );
		 ...
		 * @param Base $this The current CSS file.
		 */
		do_action_layoutbridge_adapter( "elementor/css-file/{$name}/enqueue", $this );
	}
	 ...
	 *
	 * Retrieve the name of the stylesheet used by `wp_enqueue_style_layoutbridge_adapter()`.
	 *
	 * @since 1.2.0
	 * @access protected
	 *
	...
	protected function parse_content() {
		 ...
		 * @param Base $this The current CSS file.
		 */
		do_action_layoutbridge_adapter( "elementor/{$name}-css-file/parse", $this );
		 ...
		 * @param Base $this The current CSS file.
		 */
		do_action_layoutbridge_adapter( "elementor/css-file/{$name}/parse", $this );
	...
	protected function add_repeater_control_style_rules( Controls_Stack $controls_stack, array $repeater_controls_items, array $repeater_values, array $placeholders, array $replacements ) {
		...
		foreach ( $repeater_controls_items as $index => $item ) {
			$this->add_controls_stack_style_rules(
				...
				$placeholders,
				array_merge( $replacements, [ '.drupal_layoutbuilder-repeater-item-' . $repeater_values[ $index ]['_id'] ] )
			);
		}
	}
	...
	protected function add_dynamic_control_style_rules( array $control, $value ) {
		Plugin::$instance->dynamic_tags->parse_tags_text( $value, $control, function( $id, $name, $settings ) {
			...
			$this->add_controls_stack_style_rules( $tag, $tag->get_style_controls(), $tag->get_active_settings(), [ '{{WRAPPER}}' ], [ '#drupal_layoutbuilder-tag-' . $id ] );
		} );
	}
}
