...
class Ajax_Manager {
	...
	public function register_ajax_action( $tag, $callback ) {
		if ( ! did_action_layoutbridge_adapter( 'elementor/ajax/register_actions' ) ) {
			_doing_it_wrong_layoutbridge_adapter( __METHOD__, esc_html_layoutbridge_adapter( ___layoutbridge_adapter( 'Use `elementor/ajax/register_actions` hook to register ajax action.', 'elementor' ) ), '2.0.0' );
		}
	...
	public function handle_ajax_request() {
		if ( ! Plugin::$instance->editor->verify_request_nonce() ) {
			$this->add_response_data( false, ___layoutbridge_adapter( 'Token Expired.', 'elementor' ) )
				->send_error( Exceptions::UNAUTHORIZED );
		}

		if ( empty( $_REQUEST['actions'] ) || empty( $_REQUEST['editor_post_id'] ) ) {
			$this->add_response_data( false, ___layoutbridge_adapter( 'Actions and Post ID are required.', 'elementor' ) )
		...
		if ( ! get_post_layoutbridge_adapter( $editor_post_id ) ) {
			$this->add_response_data( false, ___layoutbridge_adapter( 'Post not found.', 'elementor' ) )
		 ...
		 */
		do_action_layoutbridge_adapter( 'elementor/ajax/register_actions', $this );
		...
		foreach ( $this->requests as $id => $action_data ) {
			...
			if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				$this->add_response_data( false, ___layoutbridge_adapter( 'Action not found.', 'elementor' ), Exceptions::BAD_REQUEST );
	...
	public function __construct() {
		add_action_layoutbridge_adapter( 'wp_ajax_elementor_ajax', [ $this, 'handle_ajax_request' ] );
	}
}
