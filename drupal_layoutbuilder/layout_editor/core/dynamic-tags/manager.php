...
class Manager {
	...
	const TAG_LABEL = 'drupal_layoutbuilder-tag';
	...
	public function tag_to_text( Base_Tag $tag ) {
		return sprintf( '[%1$s id="%2$s" name="%3$s" settings="%4$s"]', self::TAG_LABEL, $tag->get_id(), $tag->get_name(), urlencode( wp_json_encode_layoutbridge_adapter( $tag->get_settings(), JSON_FORCE_OBJECT ) ) );
	}
	...
	public function get_tags() {
		if ( ! did_action_layoutbridge_adapter( 'elementor/dynamic_tags/register_tags' ) ) {
			/**
			 ...
			 */
			do_action_layoutbridge_adapter( 'elementor/dynamic_tags/register_tags', $this );
		}
	...
	public function ajax_render_tags() {
		 ...
		 * @since 2.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/dynamic_tags/before_render' );
		 ...
		 * @since 2.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/dynamic_tags/after_render' );

		wp_send_json_success_layoutbridge_adapter( $tags_data );
	}
	...
	private function add_actions() {
		add_action_layoutbridge_adapter( 'wp_ajax_elementor_render_tags', [ $this, 'ajax_render_tags' ] );
		add_action_layoutbridge_adapter( 'elementor/css-file/post/enqueue', [ $this, 'after_enqueue_post_css' ] );
	}
}
