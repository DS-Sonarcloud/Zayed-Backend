<?php
namespace DrupalLayoutbuilder\Core\DocumentTypes;
...
class Post extends Document {
		...
		return Utils::array_inject(
			...
			[
				'theme-elements-single' => [
					'title' => ___layoutbridge_adapter( 'Single', 'elementor' ),
					'active' => false,
				],
			]
		);
	...
	public static function get_title() {
		return ___layoutbridge_adapter( 'Page', 'elementor' );
	}
	...
	public function get_css_wrapper_selector() {
		return 'body.drupal_layoutbuilder-page-' . $this->get_main_id();
	}
    ...
    self::register_deploy_control( $this );
	...
	public static function register_hide_title_control( $document ) {
		...
		$page_title_selector .= ', .drupal_layoutbuilder-page-title';
      ...
      'hide_title',
      [
          'label' => ___layoutbridge_adapter('Hide Title', 'elementor'),
          'type' => Controls_Manager::SWITCHER,
          'description' => sprintf(
              /* translators: %s: Setting page link */
              ___layoutbridge_adapter(
                  'Not working? You can set a different selector for the title in the <a href="%s" target="_blank">Settings page</a>.',
                  'elementor'
              ),
  ...
  /**
	 ...
	 * @param Document $document
	 */
	public static function register_deploy_control( $document ) {
    ...
    $document->start_injection( [
		...
		$document->add_control(
			'deploy',
			[
				'label' => ___layoutbridge_adapter( 'Deploy Page', 'elementor' ),
				'type' => Controls_Manager::SWITCHER,
        'default' => 'yes', // ✅ This makes "Yes" selected by default
				'description' => sprintf(
					/* translators: %s: Setting page link */
					___layoutbridge_adapter( 'By enabling this option this page will be exported as json and will be deployed to frontend.', 'elementor' ),
					Settings::get_url() . '#tab-style'
				),
				'selectors' => [
	...
	public static function register_post_fields_control( $document ) {
		...
		if ( post_type_supports_layoutbridge_adapter( $document->post->post_type, 'excerpt' ) ) {
			$document->add_control(
				'post_excerpt',
				[
					'label' => ___layoutbridge_adapter( 'Excerpt', 'elementor' ),
		...
		if ( current_theme_supports_layoutbridge_adapter( 'post-thumbnails' ) && post_type_supports_layoutbridge_adapter( $document->post->post_type, 'thumbnail' ) ) {
			$document->add_control(
				'post_featured_image',
				[
					'label' => ___layoutbridge_adapter( 'Featured Image', 'elementor' ),
	...
	public function __construct( array $data = [] ) {
		if ( $data ) {
			$template = get_post_meta_layoutbridge_adapter( $data['post_id'], '_wp_page_template', true );
			if ( empty( $template ) ) {
				$template = 'default';
			}
