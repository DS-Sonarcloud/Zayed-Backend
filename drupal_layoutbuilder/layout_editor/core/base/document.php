...
use DrupalLayoutbuilder\DocumentsDrupal as DocumentsDrupal;
...
abstract class Document extends Controls_Stack {
	...
	public static function get_editor_panel_config() {
		return [
			...
			'messages' => [
				...
				'publish_notification' => sprintf( ___layoutbridge_adapter( 'Hurray! Your %s is live.', 'elementor' ), self::get_title() ),
			],
		];
	}
	...
	public static function get_title() {
		return ___layoutbridge_adapter( 'Document', 'elementor' );
	}
	...
	public function get_main_id() {
		...
		$parent_post_id = wp_is_post_revision_layoutbridge_adapter( $post_id );
		if ( $parent_post_id ) {
	...
	public function get_main_post() {
		return get_post_layoutbridge_adapter( $this->get_main_id() );
	}
	...
	public function get_container_classes() {
		return 'elementor drupal_layoutbuilder-' . $this->get_main_id();
	}
	 ...
	 * @access public
	 */
	public function get_wp_preview_url($post_id = NULL) {
		// $main_post_id = $this->get_main_id();
    $main_post_id = $post_id;
    $url = get_preview_post_link(
			$main_post_id,
			[
				'preview_nonce' => wp_create_nonce_layoutbridge_adapter( 'post_preview_' . $main_post_id ),
			]
		);

		/**
		 * Document "Drupal preview" URL.
		 *
		 * Filters the Drupal preview URL.
		 *
		 * @since 2.0.0
		 *
		 * @param string   $url  Drupal preview URL.
		 * @param Document $this The document instance.
		 */
		$url = apply_filters_layoutbridge_adapter( 'elementor/document/urls/wp_preview', $url, $this );
		return $url;
	}
	...
	public function get_exit_to_dashboard_url() {
		$url = get_edit_post_link_layoutbridge_adapter( $this->get_main_id(), 'raw' );
		 ...
		 * @param Document $this The document instance.
		 */
		$url = apply_filters_layoutbridge_adapter( 'elementor/document/urls/exit_to_dashboard', $url, $this );
	...
	public function get_newer_autosave() {
		...
		if ( $autosave && mysql2date( 'U', $autosave->get_post_layoutbridge_adapter()->post_modified_gmt, false ) > mysql2date( 'U', $this->post->post_modified_gmt, false ) ) {
			return $autosave;
		}
	...
	public function is_autosave() {
		return wp_is_post_autosave_layoutbridge_adapter( $this->post->ID );
	}
	...
	public function get_autosave( $user_id = 0, $create = false ) {
		if ( ! $user_id ) {
			$user_id = get_current_user_id_layoutbridge_adapter();
		}
	...
	protected function _get_initial_config() {
		return [
			...
			'urls' => [
				...
				'permalink' => $this->get_wp_preview_url(),
			],
		];
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'document_settings',
			[
				'label' => ___layoutbridge_adapter( 'General Settings', 'elementor' ),
		...
		$post_type_object = get_post_type_object_layoutbridge_adapter( $this->post->post_type );
		...
		if ( $is_published || $can_publish || ! Plugin::$instance->editor->is_edit_mode() ) {
			...
			$this->add_control(
				'post_status',
				[
					'label' => ___layoutbridge_adapter( 'Status', 'elementor' ),
					'type' => Controls_Manager::SELECT,
					'default' => $this->get_main_post()->post_status,
					'options' => get_post_statuses_layoutbridge_adapter(),
				]
			);
		}
		 ...
		 * @param Document $this The document instance.
		 */
		do_action_layoutbridge_adapter( 'elementor/documents/register_controls', $this );
	}
	...
	public function save( $data ) {
		...
		$this->post = get_post_layoutbridge_adapter( $this->post->ID );
	...
	public function is_built_with_elementor() {
		return ! ! get_post_meta_layoutbridge_adapter( $this->post->ID, '_elementor_edit_mode', true );
	}
	...
	public function get_edit_url() {
		$url = add_query_arg_layoutbridge_adapter(
			[
				'post' => $this->get_main_id(),
				'action' => 'elementor',
			],
			admin_url_layoutbridge_adapter( 'post.php' )
		);
		 ...
		 * @param Document $this The document instance.
		 */
		$url = apply_filters_layoutbridge_adapter( 'elementor/document/urls/edit', $url, $this );
	...
	public function get_preview_url() {
		...
		if ( empty( $url ) ) {
			...
			add_filter_layoutbridge_adapter( 'pre_option_permalink_structure', '__return_empty_string' );

			$url = set_url_scheme( add_query_arg_layoutbridge_adapter( [
				'drupal_layoutbuilder-preview' => $this->get_main_id(),
				'ver' => time(),
			], $this->get_permalink() ) );

			remove_filter_layoutbridge_adapter( 'pre_option_permalink_structure', '__return_empty_string' );
			 ...
			 * @param Document $this The document instance.
			 */
			$url = apply_filters_layoutbridge_adapter( 'elementor/document/urls/preview', $url, $this );
		}
	...
	public function get_json_meta( $key ) {
		if ( is_object( $this->post ) && isset( $this->post->ID ) ) {
			$meta = get_post_meta_layoutbridge_adapter( $this->post->ID, $key, true );
		} else {
			$meta = [];
		}
		...
		if ( DB::STATUS_DRAFT === $status ) {
			...
			if ( is_object( $autosave ) ) {
				$autosave_elements = Plugin::$instance->documents
					->get( $autosave->get_post_layoutbridge_adapter()->ID )
	...
	public function print_elements_with_wrapper( $elements_data = null ) {
		...
		?>
		<div class="<?php echo esc_attr_layoutbridge_adapter( $this->get_container_classes() ); ?>">
			<div class="drupal_layoutbuilder-inner">
				<div class="drupal_layoutbuilder-section-wrap">
	...
	public function get_panel_page_settings() {
		return [
			...
			'title' => sprintf( ___layoutbridge_adapter( '%s Settings', 'elementor' ), self::get_title() ),
		];
	}
	 ...
	 * @access public
	 */
	public function get_post_layoutbridge_adapter() {
		return $this->post;
	}
	...
	public function delete() {
		...
		return $deleted && ! is_wp_error_layoutbridge_adapter( $deleted );
	}
	...
	protected function save_elements( $elements ) {
		...
		$json_value = wp_slash( wp_json_encode_layoutbridge_adapter( $editor_data ) );
		 ...
		 */
		do_action_layoutbridge_adapter( 'elementor/db/before_save', $this->post->post_status, $is_meta_updated );
		 ...
		 */
		do_action_layoutbridge_adapter( 'elementor/editor/after_save', $this->post->ID, $editor_data );
	}
	...
	public function save_type() {
		update_post_meta_layoutbridge_adapter( $this->post->ID, self::TYPE_META_KEY, $this->get_name() );
	}
	...
	public function get_main_meta( $key ) {
		return get_post_meta_layoutbridge_adapter( $this->get_main_id(), $key, true );
	}
	...
	public function update_main_meta( $key, $value ) {
		return update_post_meta_layoutbridge_adapter( $this->get_main_id(), $key, $value );
	}
	...
	public function delete_main_meta( $key, $value = '' ) {
		return delete_post_meta_layoutbridge_adapter( $this->get_main_id(), $key, $value );
	}
	...
	public function get_meta( $key ) {
		return get_post_meta_layoutbridge_adapter( $this->post->ID, $key, true );
	}
	...
	public function get_last_edited() {
		...
		if ( $autosave_post ) {
			$post = $autosave_post->get_post_layoutbridge_adapter();
		}

		$date = date_i18n( _x_layoutbridge_adapter( 'M j, H:i', 'revision date format', 'elementor' ), strtotime( $post->post_modified ) );
		$display_name = get_the_author_meta( 'display_name', $post->post_author );

		if ( $autosave_post || 'revision' === $post->post_type ) {
			/* translators: 1: Saving date, 2: Author display name */
			$last_edited = sprintf( ___layoutbridge_adapter( 'Draft saved on %1$s by %2$s', 'elementor' ), '<time>' . $date . '</time>', $display_name );
		} else {
			/* translators: 1: Editing date, 2: Author display name */
			$last_edited = sprintf( ___layoutbridge_adapter( 'Last edited on %1$s by %2$s', 'elementor' ), '<time>' . $date . '</time>', $display_name );
		}
	...
	public function __construct( array $data = [] ) {
		if ( $data ) {
			if ( empty( $data['post_id'] ) ) {
				...
				$this->post = get_post_layoutbridge_adapter( $data['post_id'] );
      ...
      if(isset($this->post->ID)) {
        $saved_settings = get_post_meta_layoutbridge_adapter( $this->post->ID, '_elementor_page_settings', true );
