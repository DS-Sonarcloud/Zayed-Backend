<?php
namespace DrupalLayoutbuilder\Core\Settings\Page;

use DrupalLayoutbuilder\Core\Files\CSS\Base;
use DrupalLayoutbuilder\Core\Files\CSS\Post;
use DrupalLayoutbuilder\Core\Files\CSS\Post_Preview;
use DrupalLayoutbuilder\Core\Utils\Exceptions;
use DrupalLayoutbuilder\Core\Settings\Manager as SettingsManager;
use DrupalLayoutbuilder\Core\Settings\Base\Manager as BaseManager;
...
class Manager extends BaseManager {
	...
	public static function get_page( $id ) {
		...
		_deprecated_file( __METHOD__, '1.6.0', 'the new settings API', sprintf( 'See <a href="%s">Elementor Document Settings</a> for more information.', 'https://developers.elementor.com/drupal_layoutbuilder-document-settings/' ) );
	...
	public function get_model_for_config() {
		if ( ! is_singular_layoutbridge_adapter() && ! Plugin::$instance->editor->is_edit_mode() ) {
			return null;
		}
		...
		$model = $this->get_model( $document->get_post_layoutbridge_adapter()->ID );
	...
	public function ajax_before_save_settings( array $data, $id ) {
		$post = get_post_layoutbridge_adapter( $id );
		...
		if ( isset( $data['post_excerpt'] ) && post_type_supports_layoutbridge_adapter( $post->post_type, 'excerpt' ) ) {
		...
		if ( DB::STATUS_PUBLISH === get_post_status_layoutbridge_adapter( $id ) ) {
		...
		if ( isset( $data['post_featured_image'] ) && post_type_supports_layoutbridge_adapter( $post->post_type, 'thumbnail' ) ) {
	...
	protected function get_saved_settings( $id ) {
		$settings = get_post_meta_layoutbridge_adapter( $id, self::META_KEY, true );
		...
		if ( Utils::is_cpt_custom_templates_supported() ) {
			$saved_template = get_post_meta_layoutbridge_adapter( $id, '_wp_page_template', true );
	...
	public function save_post_status( $post_id, $status ) {
		$parent_id = wp_is_post_revision_layoutbridge_adapter( $post_id );
		...
		$post = get_post_layoutbridge_adapter( $parent_id );

		$allowed_post_statuses = get_post_statuses_layoutbridge_adapter();

		if ( isset( $allowed_post_statuses[ $status ] ) ) {
			$post_type_object = get_post_type_object_layoutbridge_adapter( $post->post_type );
