...
class Model extends BaseModel {
	...
	public function __construct( array $data = [] ) {
		$this->post = get_post_layoutbridge_adapter( $data['id'] );

		if ( ! $this->post ) {
			$this->post = new \WP_Post( (object) [] );
		}

		if ( wp_is_post_revision_layoutbridge_adapter( $this->post->ID ) ) {
			$this->post_parent = get_post_layoutbridge_adapter( $this->post->post_parent );
		} else {
	...
	public function get_panel_page_settings() {
		...
		return [
			...
			'title' => sprintf( ___layoutbridge_adapter( '%s Settings', 'elementor' ), $document::get_title() ),
		];
	}
	...
	public function on_export( $element_data ) {
		if ( ! empty( $element_data['settings']['template'] ) ) {
			/**
			 * @var \DrupalLayoutbuilder\Modules\PageTemplates\Module $page_templates_module
			 */
