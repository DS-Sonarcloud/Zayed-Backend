<?php
namespace DrupalLayoutbuilder\Core\Settings\Base;
...
abstract class Manager {
	...
	public function __construct() {
		add_action_layoutbridge_adapter( 'elementor/init', [ $this, 'on_elementor_init' ] );

		add_action_layoutbridge_adapter( 'elementor/ajax/register_actions', [ $this, 'register_ajax_actions' ] );

		$name = $this->get_css_file_name();
		add_action_layoutbridge_adapter( "elementor/css-file/{$name}/parse", [ $this, 'add_settings_css_rules' ] );
	}
	...
	final public function ajax_save_settings( $request ) {
		 ...
		 *
		 * @todo Need to be hard deprecated using `apply_filters_deprecated_layoutbridge_adapter()`.
		 *
		 * @since 1.6.0
		 ...
		 */
		$success_response_data = apply_filters_layoutbridge_adapter( "elementor/{$settings_name}/settings/success_response_data", $success_response_data, $id, $data );
		 ...
		 * @param array $data                  Settings data.
		 */
		$success_response_data = apply_filters_layoutbridge_adapter( "elementor/settings/{$settings_name}/success_response_data", $success_response_data, $id, $data );
	...
	protected function print_editor_template_content( $name ) {
		...
		<div class="drupal_layoutbuilder-panel-navigation">
			<# _.each( elementor.config.settings.<?php echo esc_html_layoutbridge_adapter( $name ); ?>.tabs, function( tabTitle, tabSlug ) { #>
				<div class="drupal_layoutbuilder-panel-navigation-tab drupal_layoutbuilder-tab-control-{{ tabSlug }}" data-tab="{{ tabSlug }}">
					<a href="#">{{{ tabTitle }}}</a>
				</div>
				<# } ); #>
		</div>
		<div id="drupal_layoutbuilder-panel-<?php echo esc_attr_layoutbridge_adapter( $name ); ?>-settings-controls"></div>
		<?php
	}
	...
	private function get_editor_template() {
		...
		ob_start();
		?>
		<script type="text/template" id="tmpl-drupal_layoutbuilder-panel-<?php echo esc_attr_layoutbridge_adapter( $name ); ?>-settings">
