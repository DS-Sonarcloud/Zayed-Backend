...
use DrupalLayoutbuilder\Core\Files\CSS\Global_CSS;
...
class Manager extends BaseManager {
		...
		foreach ( $model_controls as $tab_name => $sections ) {
			...
			foreach ( $sections as $section_name => $section_data ) {
				...
				foreach ( $section_data['controls'] as $control_name => $control_data ) {
					$saved_setting = get_option_layoutbridge_adapter( $control_name, null );

					if ( null !== $saved_setting ) {
						$settings[ $control_name ] = get_option_layoutbridge_adapter( $control_name );
					}
				}
			}
		}
	...
	protected function save_settings_to_db( array $settings, $id ) {
			...
			foreach ( $sections as $section_name => $section_data ) {
				...
				foreach ( $section_data['controls'] as $control_name => $control_data ) {
					if ( isset( $settings[ $control_name ] ) ) {
						...
						update_option_layoutbridge_adapter( $control_name, $settings[ $control_name ] );
					} else {
						delete_option_layoutbridge_adapter( $control_name );
					}
				}
			}
		}
		...
		if ( ! empty( $one_list_settings ) ) {
			update_option_layoutbridge_adapter( self::META_KEY, $one_list_settings );
		} else {
			delete_option_layoutbridge_adapter( self::META_KEY );
		}
	}
	...
	private function add_panel_tabs() {
		Controls_Manager::add_tab( self::PANEL_TAB_LIGHTBOX, ___layoutbridge_adapter( 'Lightbox', 'elementor' ) );
	}
}
