<?php
namespace DrupalLayoutbuilder\Core;

use DrupalLayoutbuilder\Core\Base\Document;
use DrupalLayoutbuilder\Core\DocumentTypes\Post;
...
class Documents_Manager {
	...
	public function __construct() {
		...
		add_action_layoutbridge_adapter( 'elementor/init', [ $this, 'register_default_types' ], 11 );
		add_action_layoutbridge_adapter( 'elementor/ajax/register_actions', [ $this, 'register_ajax_actions' ] );
	}
	...
	public function register_default_types() {
		 ...
		 */
		do_action_layoutbridge_adapter( 'elementor/documents/register', $this );
	}
	...
	public function get( $post_id, $from_cache = true ) {
		$post_id = absint_layoutbridge_adapter( $post_id );

		if ( ! $post_id || ! get_post_layoutbridge_adapter( $post_id ) ) {
			return false;
		}

		$post_id = apply_filters_layoutbridge_adapter( 'elementor/documents/get/post_id', $post_id );

		if ( ! $from_cache || ! isset( $this->documents[ $post_id ] ) ) {

			if ( wp_is_post_autosave_layoutbridge_adapter( $post_id ) ) {
				$post_type = get_post_type_layoutbridge_adapter( wp_get_post_parent_id_layoutbridge_adapter( $post_id ) );
			} else {
			...
			if ( isset( $this->cpt[ $post_type ] ) ) {
				...
				$doc_type = get_post_meta_layoutbridge_adapter( $post_id, Document::TYPE_META_KEY, true );
			}
	...
	public function get_doc_for_frontend( $post_id ) {
		if ( is_preview() || Plugin::$instance->preview->is_preview_mode() ) {
			$document = $this->get_doc_or_auto_save( $post_id, get_current_user_id_layoutbridge_adapter() );
		} else {
	...
	public function create( $type, $post_data = [], $meta_data = [] ) {
		...
		if ( empty( $post_data['post_title'] ) ) {
			$post_data['post_title'] = ___layoutbridge_adapter( 'Elementor', 'elementor' );
			if ( 'post' !== $type ) {
				$post_data['post_title'] = sprintf(
					/* translators: %s: Document title */
					___layoutbridge_adapter( 'Elementor %s', 'elementor' ),
	...
	public function ajax_save( $request ) {
		...
		Plugin::$instance->db->switch_to_post( $document->get_post_layoutbridge_adapter()->ID );
		...
		if ( DB::STATUS_AUTOSAVE === $status ) {
			...
			if ( in_array( $document->get_post_layoutbridge_adapter()->post_status, [ DB::STATUS_PUBLISH, DB::STATUS_PRIVATE ], true ) ) {
		...
		// Refresh after save.
		$document = $this->get( $document->get_post_layoutbridge_adapter()->ID, false );
    $post_id = $document->get_post_layoutbridge_adapter()->ID ?? null;
		$return_data = [
			'config' => [
				'last_edited' => $document->get_last_edited(),
				'wp_preview' => [
					'url' => $document->get_wp_preview_url($post_id),
				],
			],
		];
		 ...
		 */
		$return_data = apply_filters_layoutbridge_adapter( 'elementor/documents/ajax_save/return_data', $return_data, $document );
