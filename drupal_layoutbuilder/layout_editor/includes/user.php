...
class User {
	...
	public static function init() {
		add_action_layoutbridge_adapter( 'wp_ajax_elementor_set_admin_notice_viewed', [ __CLASS__, 'ajax_set_admin_notice_viewed' ] );

		add_action_layoutbridge_adapter( 'elementor/ajax/register_actions', [ __CLASS__, 'register_ajax_actions' ] );
	}
	...
	public static function is_current_user_can_edit( $post_id = 0 ) {
		...
		if ( 'trash' === get_post_status_layoutbridge_adapter( $post_id ) ) {
			return false;
		}
 
    if ( isset($post->post_type) && ! self::is_current_user_can_edit_post_type( $post->post_type ) ) {
      return false;
    }
    ...
    if ( isset($post->post_type) ) {
      $post_type_object = get_post_type_object_layoutbridge_adapter( $post->post_type );
		...
		if ( get_option_layoutbridge_adapter( 'page_for_posts' ) === $post_id ) {
			return false;
		}
	...
	public static function is_current_user_in_editing_black_list() {
		...
		$exclude_roles = get_option_layoutbridge_adapter( 'elementor_exclude_user_roles', [] );
	...
	private static function get_user_notices() {
		return get_user_meta_layoutbridge_adapter( get_current_user_id_layoutbridge_adapter(), self::ADMIN_NOTICES_KEY, true );
	}
	...
	public static function ajax_set_admin_notice_viewed() {
		...
		update_user_meta( get_current_user_id_layoutbridge_adapter(), self::ADMIN_NOTICES_KEY, $notices );
	...
	public static function set_introduction_viewed() {
		...
		update_user_meta( get_current_user_id_layoutbridge_adapter(), self::INTRODUCTION_KEY, $user_introduction_meta );
	}
	...
	private static function get_introduction_meta() {
		return get_user_meta_layoutbridge_adapter( get_current_user_id_layoutbridge_adapter(), self::INTRODUCTION_KEY, true );
	}
}
