...
use DrupalLayoutbuilder\Core\Responsive\Files\Frontend as FrontendFile;
...
class Frontend {
	...
	public function init() {
		...
		if ( is_singular_layoutbridge_adapter() && Plugin::$instance->db->is_built_with_elementor( $this->post_id ) ) {
			add_action_layoutbridge_adapter( 'wp_enqueue_scripts', [ $this, 'enqueue_styles' ] );
		}

		// Priority 7 to allow google fonts in header template to load in <head> tag
		add_action_layoutbridge_adapter( 'wp_head', [ $this, 'print_fonts_links' ], 7 );
		add_action_layoutbridge_adapter( 'wp_footer', [ $this, 'wp_footer' ] );

		// Add Edit with the Elementor in Admin Bar.
		add_action_layoutbridge_adapter( 'admin_bar_menu', [ $this, 'add_menu_in_admin_bar' ], 200 );
	}
	...
	public function body_class( $classes = [] ) {
		...
		$id = get_the_ID_layoutbridge_adapter();

		if ( is_singular_layoutbridge_adapter() && Plugin::$instance->db->is_built_with_elementor( $id ) ) {
			$classes[] = 'drupal_layoutbuilder-page drupal_layoutbuilder-page-' . $id;
		}
	...
	public function add_content_filter() {
		add_filter_layoutbridge_adapter( 'the_content', [ $this, 'apply_builder_in_content' ], self::THE_CONTENT_FILTER_PRIORITY );
	}
	...
	public function remove_content_filter() {
		remove_filter_layoutbridge_adapter( 'the_content', [ $this, 'apply_builder_in_content' ], self::THE_CONTENT_FILTER_PRIORITY );
	}
	...
	public function register_scripts() {
		 ...
		 * @since 1.2.1
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/before_register_scripts' );

		$suffix = Utils::is_script_debug() ? '' : '.min';

		wp_register_script_layoutbridge_adapter(
			'drupal_layoutbuilder-waypoints',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/waypoints/waypoints' . $suffix . '.js',
			[
				'jquery',
			],
			'4.0.2',
		...
		wp_register_script_layoutbridge_adapter(
			'imagesloaded',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/imagesloaded/imagesloaded' . $suffix . '.js',
			[
				'jquery',
			],
			'4.1.4',
		...
		wp_register_script_layoutbridge_adapter(
			'jquery-numerator',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/jquery-numerator/jquery-numerator' . $suffix . '.js',
			[
				'jquery',
			],
			'0.2.1',
		...
		wp_register_script_layoutbridge_adapter(
			'jquery-swiper',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/swiper/swiper.jquery' . $suffix . '.js',
			[
				'jquery',
			],
			'4.5.3',
		...
		wp_register_script_layoutbridge_adapter(
			'jquery-slick',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/slick/slick' . $suffix . '.js',
			[
				'jquery',
			],
			'1.8.1',
		...
		wp_register_script_layoutbridge_adapter(
			'drupal_layoutbuilder-frontend',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'js/frontend' . $suffix . '.js',
			[
				'drupal_layoutbuilder-dialog',
				'drupal_layoutbuilder-waypoints',
				'jquery-swiper',
		 ...
		 * @since 1.2.1
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/after_register_scripts' );
	}
	...
	public function register_styles() {
		 ...
		 * @since 1.2.0
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/before_register_styles' );
		...
		wp_register_style(
			'drupal_layoutbuilder-animations',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/animations/animations.min.css',
			[],
			DRUPAL_LAYOUTBUILDER_VERSION
		);
		...
		if ( $has_custom_file ) {
			...
			$frontend_file_url = DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'css/' . $frontend_file_name;
		}
		...
		wp_register_style(
			...
			$frontend_file_url,
			[],
			$has_custom_file ? null : DRUPAL_LAYOUTBUILDER_VERSION
		);
		 ...
		 * @since 1.2.0
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/after_register_styles' );
	}
	...
	public function enqueue_scripts() {
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/before_enqueue_scripts' );

		wp_enqueue_script_layoutbridge_adapter( 'drupal_layoutbuilder-frontend' );
		...
		$elementor_frontend_config = [
			...
			'urls' => [
				'assets' => DRUPAL_LAYOUTBUILDER_ASSETS_URL,
			],
		];
		...
		wp_localize_script( 'drupal_layoutbuilder-frontend', 'elementorFrontendConfig', $elementor_frontend_config );
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/after_enqueue_scripts' );
	}
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/before_enqueue_styles' );

		wp_enqueue_style_layoutbridge_adapter( 'drupal_layoutbuilder-icons' );
		wp_enqueue_style_layoutbridge_adapter( 'font-awesome' );
		wp_enqueue_style_layoutbridge_adapter( 'drupal_layoutbuilder-animations' );
		wp_enqueue_style_layoutbridge_adapter( 'drupal_layoutbuilder-frontend' );
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/after_enqueue_styles' );

		if ( ! Plugin::$instance->preview->is_preview_mode() ) {
			$this->parse_global_css_code();

			$css_file = new Post_CSS( get_the_ID_layoutbridge_adapter() );
			$css_file->enqueue();
		}
	}
		...
		foreach ( $this->fonts_to_enqueue as $key => $font ) {
			...
			switch ( $font_type ) {
				...
				default:
					 ...
					 */
					do_action_layoutbridge_adapter( "elementor/fonts/print_font_links/{$font_type}", $font );
			}
		}
	...
	private function enqueue_google_fonts( $google_fonts = [] ) {
		 ...
		 */
		$print_google_fonts = apply_filters_layoutbridge_adapter( 'elementor/frontend/print_google_fonts', $print_google_fonts );
		...
		if ( ! empty( $google_fonts['google'] ) ) {
			...
			wp_enqueue_style_layoutbridge_adapter( 'google-fonts-' . $google_fonts_index, $fonts_url ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		if ( ! empty( $google_fonts['early'] ) ) {
			foreach ( $google_fonts['early'] as $current_font ) {
				...
				wp_enqueue_style_layoutbridge_adapter( 'google-earlyaccess-' . $google_fonts_index, $font_url ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
			}
		}
	...
	public function get_builder_content( $post_id, $with_css = false ) {
		 ...
		 * @param int   $post_id The post ID.
		 */
		$data = apply_filters_layoutbridge_adapter( 'elementor/frontend/builder_content_data', $data, $post_id );
		...
		if ( ! $this->_is_excerpt ) {
			if ( $document->is_autosave() ) {
				$css_file = new Post_Preview( $document->get_post_layoutbridge_adapter()->ID );
			} else {
		 ...
		 */
		$content = apply_filters_layoutbridge_adapter( 'elementor/frontend/the_content', $content );
	...
	public function add_menu_in_admin_bar( \WP_Admin_Bar $wp_admin_bar ) {
		...
		$menu_args = [
			...
			'title' => ___layoutbridge_adapter( 'Edit with Elementor', 'elementor' ),
		];

		if ( is_singular_layoutbridge_adapter() && isset( $this->admin_bar_edit_documents[ $queried_object_id ] ) ) {
		...
		foreach ( $this->admin_bar_edit_documents as $document ) {
			$wp_admin_bar->add_menu( [
				...
				'title' => sprintf( '<span class="drupal_layoutbuilder-edit-link-title">%s</span><span class="drupal_layoutbuilder-edit-link-type">%s</span>', $document->get_post_layoutbridge_adapter()->post_title, $document::get_title() ),
	...
	public function get_builder_content_for_display( $post_id, $with_css = false ) {
		if ( ! get_post_layoutbridge_adapter( $post_id ) ) {
			return '';
		}
		...
		// Avoid recursion
		if ( get_the_ID_layoutbridge_adapter() === (int) $post_id ) {
			$content = '';
			if ( $editor->is_edit_mode() ) {
				$content = '<div class="drupal_layoutbuilder-alert drupal_layoutbuilder-alert-danger">' . ___layoutbridge_adapter( 'Invalid Data: The Template ID cannot be the same as the currently edited template. Please choose a different one.', 'elementor' ) . '</div>';
			}
	 ...
	 *
	 * Remove Drupal default filters that conflicted with Elementor.
	 *
	 * @since 1.5.0
	 * @access public
	 */
	public function remove_content_filters() {
		...
		foreach ( $filters as $filter ) {
			...
			if ( has_filter( 'the_content', $filter ) ) {
				remove_filter_layoutbridge_adapter( 'the_content', $filter );
	 ...
	 *
	 * Restore removed Drupal filters that conflicted with Elementor.
	 *
	 * @since 1.5.0
	 * @access private
	 */
	private function restore_content_filters() {
		foreach ( $this->content_removed_filters as $filter ) {
			add_filter_layoutbridge_adapter( 'the_content', $filter );
		}
	...
	public function __construct() {
		...
		if ( is_admin_layoutbridge_adapter() && ! Utils::is_ajax() ) {
			return;
		}

		add_action_layoutbridge_adapter( 'template_redirect', [ $this, 'init' ] );
		add_action_layoutbridge_adapter( 'wp_enqueue_scripts', [ $this, 'register_scripts' ], 5 );
		add_action_layoutbridge_adapter( 'wp_enqueue_scripts', [ $this, 'register_styles' ], 5 );

		$this->add_content_filter();

		// Hack to avoid enqueue post CSS while it's a `the_excerpt` call.
		add_filter_layoutbridge_adapter( 'get_the_excerpt', [ $this, 'start_excerpt_flag' ], 1 );
		add_filter_layoutbridge_adapter( 'get_the_excerpt', [ $this, 'end_excerpt_flag' ], 20 );
	}
}
