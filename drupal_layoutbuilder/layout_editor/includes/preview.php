...
class Preview {
	...
	public function init() {
		if ( is_admin_layoutbridge_adapter() || ! $this->is_preview_mode() ) {
			return;
		}

		$this->post_id = get_the_ID_layoutbridge_adapter();

		// Don't redirect to permalink.
		remove_action_layoutbridge_adapter( 'template_redirect', 'redirect_canonical' );
		...
		if ( class_exists( 'WPSEO_Frontend' ) ) {
			remove_action_layoutbridge_adapter( 'template_redirect', [ \WPSEO_Frontend::get_instance(), 'clean_permalink' ], 1 );
		}

		// Disable the WP admin bar in preview mode.
		add_filter_layoutbridge_adapter( 'show_admin_bar', '__return_false' );

		add_action_layoutbridge_adapter( 'wp_enqueue_scripts', function() {
			$this->enqueue_styles();
			$this->enqueue_scripts();
		} );

		add_filter_layoutbridge_adapter( 'the_content', [ $this, 'builder_wrapper' ], 999999 );
		 ...
		 */
		do_action_layoutbridge_adapter( 'elementor/preview/init', $this );
	}
	...
	public function is_preview_mode( $post_id = 0 ) {
		...
		if ( ! isset( $_GET['drupal_layoutbuilder-preview'] ) || $post_id !== (int) $_GET['drupal_layoutbuilder-preview'] ) {
			return false;
		}
	...
	public function builder_wrapper( $content ) {
		if ( get_the_ID_layoutbridge_adapter() === $this->post_id ) {
			$classes = 'drupal_layoutbuilder-edit-mode';
	...
	private function enqueue_styles() {
		...
		wp_register_style(
			'editor-preview',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'css/editor-preview' . $direction_suffix . $suffix . '.css',
			[
				'drupal_layoutbuilder-select2',
			],
		...
		wp_enqueue_style_layoutbridge_adapter( 'editor-preview' );
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/preview/enqueue_styles' );
	}
	...
	private function enqueue_scripts() {
		...
		wp_enqueue_script_layoutbridge_adapter(
			'drupal_layoutbuilder-inline-editor',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/inline-editor/js/inline-editor' . $suffix . '.js',
			[],
			DRUPAL_LAYOUTBUILDER_VERSION,
			true
		);
		 ...
		 * @since 1.5.4
		 */
		do_action_layoutbridge_adapter( 'elementor/preview/enqueue_scripts' );
	}
	...
	public function __construct() {
		add_action_layoutbridge_adapter( 'template_redirect', [ $this, 'init' ], 0 );
	}
}
