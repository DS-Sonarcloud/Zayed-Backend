 ...
 * heartbeat. The class communicates with Drupal Heartbeat API while working
 * with Elementor.
 *
 * @since 1.0.0
 */
class Heartbeat {
	...
	public function heartbeat_received( $response, $data ) {
		if ( isset( $data['elementor_post_lock']['post_ID'] ) ) {
			...
			$response['elementorNonce'] = Plugin::$instance->editor->create_nonce( get_post_type_layoutbridge_adapter( $post_id ) );
		}
		return $response;
	}
	...
	public function refresh_nonces( $response, $data ) {
		if ( isset( $data['elementor_post_lock']['post_ID'] ) ) {
			$post_type = get_post_type_layoutbridge_adapter( $data['elementor_post_lock']['post_ID'] );
			$response['drupal_layoutbuilder-refresh-nonces'] = [
				'elementorNonce' => Plugin::$instance->editor->create_nonce( $post_type ),
				'heartbeatNonce' => wp_create_nonce_layoutbridge_adapter( 'heartbeat-nonce' ),
			];
		}
	...
	public function __construct() {
		add_filter_layoutbridge_adapter( 'heartbeat_received', [ $this, 'heartbeat_received' ], 10, 2 );
		add_filter_layoutbridge_adapter( 'wp_refresh_nonces', [ $this, 'refresh_nonces' ], 30, 2 );
	}
}
