...
class DB {
	...
	protected function _get_json_meta( $post_id, $key ) {
		$meta = get_post_meta_layoutbridge_adapter( $post_id, $key, true );
	...
	/**
	 * Get new editor from Drupal editor.
	 *
	...
	public function set_is_elementor_page( $post_id, $is_elementor = true ) {
		if ( $is_elementor ) {
			...
			update_post_meta_layoutbridge_adapter( $post_id, '_elementor_edit_mode', 'builder' );
		} else {
			delete_post_meta_layoutbridge_adapter( $post_id, '_elementor_edit_mode' );
		}
	}
	...
	public function safe_copy_elementor_meta( $from_post_id, $to_post_id ) {
		...
		if ( ! did_action_layoutbridge_adapter( 'elementor/db/before_save' ) ) {
			...
			if ( get_post_meta_layoutbridge_adapter( $to_post_id, '_elementor_data', true ) ) {
				return;
			}
		}
	...
	public function copy_elementor_meta( $from_post_id, $to_post_id ) {
		$from_post_meta = get_post_meta_layoutbridge_adapter( $from_post_id );
		$core_meta = [
			'_wp_page_template',
			'_thumbnail_id',
		];
	...
	public function is_built_with_elementor( $post_id ) {
		return ! ! get_post_meta_layoutbridge_adapter( $post_id, '_elementor_edit_mode', true );
	}

	/**
	 * Switch to post.
	 *
	 * Change the global Drupal post to the requested post.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	...
	public function switch_to_post( $post_id ) {
		...
		if ( get_the_ID_layoutbridge_adapter() === $post_id ) {
		...
		$this->switched_post_data[] = [
			'switched_id' => $post_id,
			'original_id' => get_the_ID_layoutbridge_adapter(), // Note, it can be false if the global isn't set
		];

		$GLOBALS['post'] = get_post_layoutbridge_adapter( $post_id ); // WPCS: override ok.

		setup_postdata_layoutbridge_adapter( $GLOBALS['post'] );
	}
	...
	public function restore_current_post() {
		...
		$GLOBALS['post'] = get_post_layoutbridge_adapter( $data['original_id'] ); // WPCS: override ok.
	 ...
	 * Switch to query.
	 *
	 * Change the Drupal query to a new query with the requested
	 * query variables.
	 *
	 * @since 2.0.0
	 * @access public
	...
	public function switch_to_query( $query_vars ) {
		...
		if ( $new_query->is_singular_layoutbridge_adapter() && isset( $new_query->posts[0] ) ) {
	...
	public function restore_current_query() {
		...
		if ( $wp_query->is_singular_layoutbridge_adapter() && isset( $wp_query->posts[0] ) ) {
