...
class Api {
	...
	private static function get_info_data( $force_update = false ) {
		$cache_key = 'elementor_remote_info_api_data_' . DRUPAL_LAYOUTBUILDER_VERSION;

		$info_data = get_transient_layoutbridge_adapter( $cache_key );

		if ( $force_update || false === $info_data ) {
			$timeout = ( $force_update ) ? 25 : 8;

			$response = wp_remote_post_layoutbridge_adapter( self::$api_info_url, [
				'timeout' => $timeout,
				'body' => [
					// Which API version is used.
					'api_version' => DRUPAL_LAYOUTBUILDER_VERSION,
					// Which language to return.
					'site_lang' => get_bloginfo_layoutbridge_adapter( 'language' ),
				],
			] );

			if ( is_wp_error_layoutbridge_adapter( $response ) || 200 !== (int) wp_remote_retrieve_response_code_layoutbridge_adapter( $response ) ) {
				set_transient_layoutbridge_adapter( $cache_key, [], 2 * HOUR_IN_SECONDS );

				return false;
			}

			$info_data = json_decode( wp_remote_retrieve_body_layoutbridge_adapter( $response ), true );
			...
			if ( isset( $info_data['library'] ) ) {
				...
				update_option_layoutbridge_adapter( self::LIBRARY_OPTION_KEY, $info_data['library'], 'no' );

				unset( $info_data['library'] );
			}

			if ( isset( $info_data['feed'] ) ) {
				update_option_layoutbridge_adapter( self::FEED_OPTION_KEY, $info_data['feed'], 'no' );

				unset( $info_data['feed'] );
			}

			set_transient_layoutbridge_adapter( $cache_key, $info_data, 12 * HOUR_IN_SECONDS );
		}
	...
	public static function get_library_data( $force_update = false ) {
		...
		$library_data = get_option_layoutbridge_adapter( self::LIBRARY_OPTION_KEY );
	...
	public static function get_feed_data( $force_update = false ) {
		...
		$feed = get_option_layoutbridge_adapter( self::FEED_OPTION_KEY );
	...
	public static function get_template_content( $template_id ) {
		 ...
		 */
		$body_args = apply_filters_layoutbridge_adapter( 'elementor/api/get_templates/body_args', $body_args );

		$response = wp_remote_get_layoutbridge_adapter( $url, [
			'timeout' => 40,
			'body' => $body_args,
		] );

		if ( is_wp_error_layoutbridge_adapter( $response ) ) {
			return $response;
		}

		$response_code = (int) wp_remote_retrieve_response_code_layoutbridge_adapter( $response );

		if ( 200 !== $response_code ) {
			return new \WP_Error( 'response_code_error', sprintf( 'The request returned with a status code of %s.', $response_code ) );
		}

		$template_content = json_decode( wp_remote_retrieve_body_layoutbridge_adapter( $response ), true );
	...
	public static function send_feedback( $feedback_key, $feedback_text ) {
		return wp_remote_post_layoutbridge_adapter( self::$api_feedback_url, [
			'timeout' => 30,
			'body' => [
				'api_version' => DRUPAL_LAYOUTBUILDER_VERSION,
				'site_lang' => get_bloginfo_layoutbridge_adapter( 'language' ),
	...
	public static function init() {
		add_action_layoutbridge_adapter( 'wp_ajax_elementor_reset_library', [ __CLASS__, 'ajax_reset_api_data' ] );
	}
}
