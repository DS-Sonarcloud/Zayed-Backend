...
class Embed {
	...
	public static function get_embed_url( $video_url, array $embed_url_params = [], array $options = [] ) {
		...
		return add_query_arg_layoutbridge_adapter( $embed_url_params, $embed_pattern );
	}
	...
	public static function get_embed_html( $video_url, array $embed_url_params = [], array $options = [], array $frame_attributes = [] ) {
		$default_frame_attributes = [
			'class' => 'drupal_layoutbuilder-video-iframe',
			'allowfullscreen',
		];
		...
		foreach ( $frame_attributes as $attribute_key => $attribute_value ) {
			$attribute_value = esc_attr_layoutbridge_adapter( $attribute_value );
		...
		return apply_filters_layoutbridge_adapter( 'oembed_result', $iframe_html, $video_url, $frame_attributes );
	}
}
