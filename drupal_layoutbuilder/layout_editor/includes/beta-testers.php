...
class Beta_Testers {
	...
	private function get_beta_version() {
		...
		if ( false === $beta_version ) {
			...
			$response = wp_remote_get_layoutbridge_adapter( 'https://plugins.svn.wordpress.org/elementor/trunk/readme.txt' );

			if ( ! is_wp_error_layoutbridge_adapter( $response ) && ! empty( $response['body'] ) ) {
	 ...
	 *
	 * Fired by `pre_set_site_transient_update_plugins` filter, before Drupal
	...
	public function check_version( $transient ) {
		...
		if ( 'false' !== $beta_version && version_compare( $beta_version, DRUPAL_LAYOUTBUILDER_VERSION, '>' ) ) {
			...
			$transient->response[ DRUPAL_LAYOUTBUILDER_PLUGIN_BASE ] = $response;
		}
	...
	public function __construct() {
		if ( 'yes' !== get_option_layoutbridge_adapter( 'elementor_beta', 'no' ) ) {
			return;
		}

		$this->transient_key = md5( 'elementor_beta_testers_response_key' );

		add_filter_layoutbridge_adapter( 'pre_set_site_transient_update_plugins', [ $this, 'check_version' ] );
	}
}
