...
class Control_Drupal_Gallery extends Control_Base_Multiple
{
	...
	{ ?>
		<div class="drupal_layoutbuilder-control-field drupal-gallery-control">
			<div class="drupal_layoutbuilder-control-title">{{{ data.label }}}</div>
					...
					<# if ( data.controlValue && data.controlValue.length ) { #>
						...
						<# } else { #>
							<div class="drupal-gallery-placeholder select-drupal-gallery">
								...
								<span><?php echo ___layoutbridge_adapter('No images selected', 'elementor'); ?></span>
							</div>
							<# } #>
				</div>

				<div class="drupal-gallery-actions">
					<button class="drupal_layoutbuilder-button select-drupal-gallery">
						<i class="eicon-edit"></i> <?php echo ___layoutbridge_adapter('Add / Edit Gallery', 'elementor'); ?>
					</button>
					<button class="drupal_layoutbuilder-button drupal_layoutbuilder-button-danger clear-drupal-gallery">
						<i class="eicon-trash"></i> <?php echo ___layoutbridge_adapter('Clear', 'elementor'); ?>
					</button>
				</div>
		...
		<style>
			...
			.drupal-gallery-actions .drupal_layoutbuilder-button {
				flex: 1;
				padding: 6px 10px;
				font-size: 13px;
				display: flex;
			...
			.drupal-gallery-actions .drupal_layoutbuilder-button i {
				font-size: 14px;
			}

			/* --- Elementor Drupal Gallery Layout --- */
			.drupal_layoutbuilder-drupal-gallery .gallery {
				display: grid;
				gap: 12px;
			}

			.drupal_layoutbuilder-drupal-gallery .gallery-item {
				position: relative;
				overflow: hidden;
				border-radius: 8px;
			...
			.drupal_layoutbuilder-drupal-gallery .gallery-item:hover {
				transform: scale(1.02);
			}

			.drupal_layoutbuilder-drupal-gallery .gallery-item img {
				width: 100%;
				height: 100%;
				object-fit: cover;
				display: block;
