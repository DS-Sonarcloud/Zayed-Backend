...
class Control_Color extends Base_Data_Control {
	...
	public function enqueue() {
		...
		wp_register_script_layoutbridge_adapter(
			...
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/wp-color-picker/wp-color-picker-alpha' . $suffix . '.js',
			[
				'wp-color-picker',
			],
			'2.0.1',
		...
		wp_enqueue_style_layoutbridge_adapter( 'wp-color-picker' );
		wp_enqueue_script_layoutbridge_adapter( 'wp-color-picker-alpha' );
	}
	...
	public function content_template() {
		...
		<div class="drupal_layoutbuilder-control-field">
			<label class="drupal_layoutbuilder-control-title">
				<# if ( data.label ) { #>
					{{{ data.label }}}
				<# } #>
				<# if ( data.description ) { #>
					<span class="drupal_layoutbuilder-control-field-description">{{{ data.description }}}</span>
				<# } #>
			</label>
			<div class="drupal_layoutbuilder-control-input-wrapper">
				<input data-setting="{{ name }}" type="text" placeholder="<?php echo esc_attr_layoutbridge_adapter( 'Hex/rgba', 'elementor' ); ?>" {{ defaultValue }}{{ dataAlpha }} />
			</div>
		</div>
		<?php
	}
