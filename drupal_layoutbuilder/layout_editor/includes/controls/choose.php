	...
	public function content_template() {
		...
		<div class="drupal_layoutbuilder-control-field">
			...
			<div class="drupal_layoutbuilder-control-input-wrapper">
				<div class="drupal_layoutbuilder-choices">
					<# _.each( data.options, function( options, value ) { #>
					<input id="<?php echo $control_uid; ?>" type="radio" name="drupal_layoutbuilder-choose-{{ data.name }}-{{ data._cid }}" value="{{ value }}">
					<label class="drupal_layoutbuilder-choices-label tooltip-target" for="<?php echo $control_uid; ?>" data-tooltip="{{ options.title }}" title="{{ options.title }}">
						<i class="{{ options.icon }}" aria-hidden="true"></i>
						<span class="drupal_layoutbuilder-screen-only">{{{ options.title }}}</span>
					</label>
					<# } ); #>
				</div>
			</div>
