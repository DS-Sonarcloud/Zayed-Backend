...
abstract class Control_Base_Units extends Control_Base_Multiple {
	...
	protected function print_units_template() {
		...
		<div class="drupal_layoutbuilder-units-choices">
			<# _.each( data.size_units, function( unit ) { #>
			<input id="drupal_layoutbuilder-choose-{{ data._cid + data.name + unit }}" type="radio" name="drupal_layoutbuilder-choose-{{ data.name }}" data-setting="unit" value="{{ unit }}">
			<label class="drupal_layoutbuilder-units-choices-label" for="drupal_layoutbuilder-choose-{{ data._cid + data.name + unit }}">{{{ unit }}}</label>
			<# } ); #>
		</div>
		<# } #>
		<?php
