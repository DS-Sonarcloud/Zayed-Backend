...
class Group_Control_Image_Size extends Group_Control_Base
{
	...
	public static function get_attachment_image_html($settings, $image_size_key = 'image', $image_key = null)
	{
		...
		$image_class = ! empty($settings['hover_animation']) ? 'drupal_layoutbuilder-animation-' . $settings['hover_animation'] : '';
		$html = '';

		// If is the new version - with image size.
		$image_sizes = get_intermediate_image_sizes_layoutbridge_adapter();
		// Load all image styles
		$image_styles = ImageStyle::loadMultiple();
		$options = [];

		// Prepare options array for dropdown: machine_name => label
		foreach ($image_styles as $machine_name => $style) {
			$options[$machine_name] = $style->label();
		}

		$image_sizes[] = 'full';
		$effects = $image_styles[$size]->getEffects();
		$image_size_attr['width'] = '';
		$image_size_attr['height'] = '';
		foreach ($effects as $effect) {
			$effect_id = $effect->getPluginId();
			$effect_data = $effect->getConfiguration();

			if ($effect_id === 'image_scale' || $effect_id === 'image_resize') {
				$image_size_attr['width'] = $effect_data['data']['width'] ?? '';
				$image_size_attr['height'] = $effect_data['data']['height'] ?? '';
				break; // Mil gaya, ab aur check karne ki zarurat nahi
			}
		}

		if (! empty($image['id'])) {
			$image_class .= " attachment-$size size-$size image-style-$size";
			$attributes = [
				'class' => $image_class,
				'width' => $image_size_attr['width'],
				'height' => $image_size_attr['height'],
				'style' => $image_size_attr['width'] && $image_size_attr['height'] ? 'width: ' . $image_size_attr['width'] . 'px; height: ' . $image_size_attr['height'] . 'px;' : '',
			];
			$html .= wp_get_attachment_image_layoutbridge_adapter($image['id'], $size, false, $attributes);
		} else {
			...
			if (! empty($image_src)) {
				...
				$html .= sprintf('<img src="%s" title="%s" alt="%s"%s />', esc_attr_layoutbridge_adapter($image_src), Control_Media::get_image_title($image), Control_Media::get_image_alt($image), $image_class_html);
			}
		}
	...
	public static function get_all_image_sizes()
	{
		...
		foreach ($default_image_sizes as $size) {
			$image_sizes[$size] = [
				'width' => (int) get_option_layoutbridge_adapter($size . '_size_w'),
				'height' => (int) get_option_layoutbridge_adapter($size . '_size_h'),
				'crop' => (bool) get_option_layoutbridge_adapter($size . '_crop'),
			];
		}
		...
		return apply_filters_layoutbridge_adapter('image_size_names_choose', $image_sizes);
	}
	...
	public static function get_attachment_image_src($attachment_id, $image_size_key, array $settings)
	{
		...
		if ('custom' !== $size) {
			...
			require_once(DRUPAL_LAYOUTBUILDER_PATH . 'includes/libraries/bfi-thumb/bfi-thumb.php');
		...
		$image_src = wp_get_attachment_image_src_layoutbridge_adapter($attachment_id, $attachment_size);
	...
	protected function init_fields()
	{
		...
		$fields['size'] = [
			'label' => _x_layoutbridge_adapter('Image Size', 'Image Size Control', 'elementor'),
			'type' => Controls_Manager::SELECT,
			'label_block' => false,
		];

		$fields['custom_dimension'] = [
			'label' => _x_layoutbridge_adapter('Image Dimension', 'Image Size Control', 'elementor'),
			'type' => Controls_Manager::IMAGE_DIMENSIONS,
			'description' => ___layoutbridge_adapter('You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'elementor'),
			'condition' => [
				'size' => 'custom',
			],
			'separator' => 'none',
	...
	protected function prepare_fields($fields)
	{
		...
		if (! empty($args['default']) && isset($options[$args['default']])) {
			$default_value = $args['default'];
		} else {
			// Get the first available image style
			$default_keys = array_keys($options);
			$default_value = array_shift($default_keys);
		}

		// Apply styles to 'size' field
		$fields['size']['options'] = $options;
		$fields['size']['default'] = $default_value;

		// Hide 'custom_dimension' field if 'custom' style is not present
	...
	private function get_image_sizes()
	{
		...
		$image_sizes['full'] = _x_layoutbridge_adapter('Full', 'Image Size Control', 'elementor');

		if (! empty($args['include']['custom']) || ! in_array('custom', $args['exclude'])) {
			$image_sizes['custom'] = _x_layoutbridge_adapter('Custom', 'Image Size Control', 'elementor');
		}
