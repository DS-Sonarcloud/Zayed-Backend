...
class Utils {
	 ...
	 * Is ajax.
	 *
	 * Whether the current request is a Drupal ajax request.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	 *
	 * @return bool True if it's a Drupal ajax request, false otherwise.
	 */
	public static function is_ajax() {
		// TODO: When minimum required version of Drupal will be 4.7, use `wp_doing_ajax_layoutbridge_adapter()` instead.
	...
	public static function get_edit_link( $post_id = 0 ) {
		 ...
		 */
		$edit_link = apply_filters_layoutbridge_adapter( 'elementor/utils/get_edit_link', $edit_link, $post_id );
	...
	public static function get_pro_link( $link ) {
		...
		if ( ! $theme_name ) {
			$theme_obj = wp_get_theme_layoutbridge_adapter();
			if ( $theme_obj->parent() ) {
			...
			$theme_name = sanitize_key_layoutbridge_adapter( $theme_name );
		}

		$link = add_query_arg_layoutbridge_adapter( 'utm_term', $theme_name, $link );

		if ( defined( 'DRUPAL_LAYOUTBUILDER_PARTNER_ID' ) ) {
			$link = add_query_arg_layoutbridge_adapter( 'partner_id', sanitize_key_layoutbridge_adapter( DRUPAL_LAYOUTBUILDER_PARTNER_ID ), $link );
		}
	...
	public static function get_preview_url( $post_id ) {
		 ...
		 * @param int    $post_id     Post ID.
		 */
		$url = apply_filters_layoutbridge_adapter( 'elementor/utils/preview_url', $url, $post_id );

		return $url;
	}

	/**
	 * Get Drupal preview url.
	 *
	 * Retrieve Drupal preview URL for any given post ID.
	 *
	 * @since 1.9.0
	 ...
	 * @param int $post_id Post ID.
	 *
	 * @return string Drupal preview URL.
	 */
	public static function get_wp_preview_url( $post_id ) {
		 ...
		 *
		 * @param string $wp_preview_url Drupal preview URL.
		 * @param int    $post_id        Post ID.
		 */
		$wp_preview_url = apply_filters_layoutbridge_adapter( 'elementor/utils/wp_preview_url', $wp_preview_url, $post_id );
	...
	public static function replace_urls( $from, $to ) {
		...
		if ( $from === $to ) {
			throw new \Exception( ___layoutbridge_adapter( 'The `from` and `to` URL\'s must be different', 'elementor' ) );
		}

		$is_valid_urls = ( filter_var( $from, FILTER_VALIDATE_URL ) && filter_var( $to, FILTER_VALIDATE_URL ) );
		if ( ! $is_valid_urls ) {
			throw new \Exception( ___layoutbridge_adapter( 'The `from` and `to` URL\'s must be valid URL\'s', 'elementor' ) );
		}
		...
		if ( false === $rows_affected ) {
			throw new \Exception( ___layoutbridge_adapter( 'An error occurred', 'elementor' ) );
		}
		...
		return sprintf(
			...
			_n_layoutbridge_adapter( '%d row affected.', '%d rows affected.', $rows_affected, 'elementor' ),
			$rows_affected
		);
	}
	...
	public static function is_post_support( $post_id = 0 ) {
		$post_type = get_post_type_layoutbridge_adapter( $post_id );
		 ...
		 */
		$is_supported = apply_filters_layoutbridge_adapter( 'elementor/utils/is_post_type_support', $is_supported, $post_id, $post_type );
		 ...
		 * @param string $post_type Post type.
		 */
		$is_supported = apply_filters_layoutbridge_adapter( 'elementor/utils/is_post_support', $is_supported, $post_id, $post_type );
	...
	public static function is_post_type_support( $post_type ) {
		if ( ! post_type_exists_layoutbridge_adapter( $post_type ) ) {
			return false;
		}

		if ( ! post_type_supports_layoutbridge_adapter( $post_type, 'elementor' ) ) {
			return false;
		}
	...
	public static function get_placeholder_image_src() {
		$placeholder_image = DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'images/placeholder.png';
		 ...
		 */
		$placeholder_image = apply_filters_layoutbridge_adapter( 'elementor/utils/get_placeholder_image_src', $placeholder_image );
	 ...
	 * Do not cache.
	 *
	 * Tell Drupal cache plugins not to cache this request.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	 ...
	 * Get timezone string.
	 *
	 * Retrieve timezone string from the Drupal database.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	...
	public static function get_timezone_string() {
		$current_offset = (float) get_option_layoutbridge_adapter( 'gmt_offset' );
		$timezone_string = get_option_layoutbridge_adapter( 'timezone_string' );
	 ...
	 * @param array  $args        Array of additional function arguments to be passed to `do_action_layoutbridge_adapter()`.
	 * @param string $version     The version of Drupal that deprecated the hook.
	 ...
	 * @access public
	 * @static
	 * @deprecated 2.1.0 Use `apply_filters_deprecated_layoutbridge_adapter()` instead
	 *
	 * @param string $tag         The name of the filter hook.
	 * @param array  $args        Array of additional function arguments to be passed to `apply_filters_layoutbridge_adapter()`.
	 * @param string $version     The version of Drupal that deprecated the hook.
	 * @param bool   $replacement Optional. The hook that should have been used.
	 * @param string $message     Optional. A message regarding the change.
	 *
	 ...
	 */
	public static function apply_filters_deprecated_layoutbridge_adapter( $tag, $args, $version, $replacement = false, $message = null ) {
		_deprecated_function( __METHOD__, '2.1.0', 'apply_filters_deprecated_layoutbridge_adapter()' );

		return apply_filters_deprecated_layoutbridge_adapter( $tag, $args, $version, $replacement, $message );
	}
	...
	public static function get_create_new_post_url( $post_type = 'page' ) {
		$new_post_url = add_query_arg_layoutbridge_adapter( [
			'action' => 'elementor_new_post',
			'post_type' => $post_type,
		], admin_url_layoutbridge_adapter( 'edit.php' ) );
	...
	public static function get_post_autosave( $post_id, $user_id = 0 ) {
		...
		$post = get_post_layoutbridge_adapter( $post_id );
	...
	public static function is_cpt_custom_templates_supported() {
		...
		return method_exists( wp_get_theme_layoutbridge_adapter(), 'get_post_templates' );
	}
