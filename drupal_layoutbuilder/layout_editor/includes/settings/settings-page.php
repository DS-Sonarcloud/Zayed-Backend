 ...
 * Drupal dashboard settings pages in inheriting classes.
 *
 * @since 1.0.0
 * @abstract
 */
abstract class Settings_Page {
	...
	final public static function get_url() {
		return admin_url_layoutbridge_adapter( 'admin.php?page=' . static::PAGE_ID );
	}
	...
	public function __construct() {
		if ( ! empty( $_POST['option_page'] ) && static::PAGE_ID === $_POST['option_page'] ) {
			add_action_layoutbridge_adapter( 'admin_init', [ $this, 'register_settings_fields' ] );
		}
	}
	...
	public function display_settings_page() {
		...
		<div class="wrap">
			...
			<div id="drupal_layoutbuilder-settings-tabs-wrapper" class="nav-tab-wrapper">
				<?php
				foreach ( $tabs as $tab_id => $tab ) {
					...
					echo "<a id='drupal_layoutbuilder-settings-tab-{$tab_id}' class='nav-tab{$active_class}' href='#tab-{$tab_id}'>{$tab['label']}</a>";
				}
				?>
			</div>
			<form id="drupal_layoutbuilder-settings-form" method="post" action="options.php">
				...
				foreach ( $tabs as $tab_id => $tab ) {
					...
					if ( 'general' === $tab_id ) {
						$active_class = ' drupal_layoutbuilder-active';
					}

					echo "<div id='tab-{$tab_id}' class='drupal_layoutbuilder-settings-form-page{$active_class}'>";
	...
	private function ensure_tabs() {
		if ( null === $this->tabs ) {
			 ...
			 */
			do_action_layoutbridge_adapter( "elementor/admin/after_create_settings/{$page_id}", $this );
		}
	}
}
