...
class Server_Reporter extends Base_Reporter {
	...
	public function get_php_version() {
		...
		if ( version_compare( $result['value'], '5.4', '<' ) ) {
			$result['recommendation'] = _x_layoutbridge_adapter( 'We recommend to use php 5.4 or higher', 'System Info', 'elementor' );
	...
	public function get_write_permissions() {
		...
		$elementor_uploads_path = $wp_upload_dir['basedir'] . '/layout_editor';
	...
	public function get_elementor_library() {
		$response = wp_remote_post_layoutbridge_adapter(
			Api::$api_info_url, [
				'timeout' => 5,
				'body' => [
		...
		$http_response_code = wp_remote_retrieve_response_code_layoutbridge_adapter( $response );
