<?php
namespace DrupalLayoutbuilder\System_Info;

use DrupalLayoutbuilder\System_Info\Classes\Abstracts\Base_Reporter;
use DrupalLayoutbuilder\System_Info\Helpers\Model_Helper;
 ...
 * reports and displaying them on Drupal dashboard under Elementor setting.
 *
 * @since 1.0.0
 */
class Main {
	...
	private function add_actions() {
		add_action_layoutbridge_adapter( 'admin_menu', [ $this, 'register_menu' ], 500 );
		add_action_layoutbridge_adapter( 'wp_ajax_elementor_system_info_download_file', [ $this, 'download_file' ] );
	}
	...
	public function display_page() {
		...
		?>
		<div id="drupal_layoutbuilder-system-info">
			<h3><?php echo ___layoutbridge_adapter( 'System Info', 'elementor' ); ?></h3>
			<div><?php $this->print_report( $reports, 'html' ); ?></div>
			<h3><?php echo ___layoutbridge_adapter( 'Copy & Paste Info', 'elementor' ); ?></h3>
			<div id="drupal_layoutbuilder-system-info-raw">
				<label id="drupal_layoutbuilder-system-info-raw-code-label" for="drupal_layoutbuilder-system-info-raw-code"><?php echo ___layoutbridge_adapter( 'You can copy the below info as simple text with Ctrl+C / Ctrl+V:', 'elementor' ); ?></label>
				<textarea id="drupal_layoutbuilder-system-info-raw-code" readonly>
					<?php
				...
				<script>
					var textarea = document.getElementById( 'drupal_layoutbuilder-system-info-raw-code' );
			...
			<hr>
			<form action="<?php echo admin_url_layoutbridge_adapter( 'admin-ajax.php' ); ?>" method="post">
				<input type="hidden" name="action" value="elementor_system_info_download_file">
				<input type="submit" class="button button-primary" value="<?php echo ___layoutbridge_adapter( 'Download System Info', 'elementor' ); ?>">
			</form>
		</div>
		<?php
	}
	...
	public function download_file() {
		if ( ! current_user_can( $this->capability ) ) {
			wp_die( ___layoutbridge_adapter( 'You don\'t have a permission to download this file', 'elementor' ) );
		}
	...
	public function register_menu() {
		$system_info_text = ___layoutbridge_adapter( 'System Info', 'elementor' );
		...
		add_submenu_page_layoutbridge_adapter(
			...
			$system_info_text,
			$this->capability,
			'drupal_layoutbuilder-system-info',
			[ $this, 'display_page' ]
		);
	}
	...
	protected function get_default_settings() {
		...
		$base_lib_dir = DRUPAL_LAYOUTBUILDER_PATH . 'includes/settings/system-info/';
