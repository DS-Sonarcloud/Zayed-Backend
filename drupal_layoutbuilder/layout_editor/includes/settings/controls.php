...
class Settings_Controls {
	...
	private static function text( array $field ) {
		...
		foreach ( $field['attributes'] as $attribute_key => $attribute_values ) {
			$attributes[] = sprintf( '%1$s="%2$s"', $attribute_key, esc_attr_layoutbridge_adapter( $attribute_values ) );
		}

		$attributes = implode( ' ', $attributes );
		?>
		<input type="<?php echo esc_attr_layoutbridge_adapter( $field['type'] ); ?>" id="<?php echo esc_attr_layoutbridge_adapter( $field['id'] ); ?>" name="<?php echo esc_attr_layoutbridge_adapter( $field['id'] ); ?>" value="<?php echo esc_attr_layoutbridge_adapter( get_option_layoutbridge_adapter( $field['id'], $field['std'] ) ); ?>" <?php echo $attributes; ?>/>
		<?php
	...
	private static function checkbox( array $field ) {
		?>
		<label>
			<input type="<?php echo esc_attr_layoutbridge_adapter( $field['type'] ); ?>" id="<?php echo esc_attr_layoutbridge_adapter( $field['id'] ); ?>" name="<?php echo esc_attr_layoutbridge_adapter( $field['id'] ); ?>" value="<?php echo $field['value']; ?>"<?php checked( $field['value'], get_option_layoutbridge_adapter( $field['id'], $field['std'] ) ); ?> />
			<?php
			if ( ! empty( $field['sub_desc'] ) ) :
				echo $field['sub_desc'];
			endif;
	...
	private static function checkbox_list( array $field ) {
		$old_value = get_option_layoutbridge_adapter( $field['id'], $field['std'] );
	...
	private static function select( array $field ) {
		$old_value = get_option_layoutbridge_adapter( $field['id'], $field['std'] );
		?>
		<select name="<?php echo esc_attr_layoutbridge_adapter( $field['id'] ); ?>">
			<?php if ( ! empty( $field['show_select'] ) ) : ?>
				<option value="">— <?php echo ___layoutbridge_adapter( 'Select', 'elementor' ); ?> —</option>
			<?php endif; ?>

			<?php foreach ( $field['options'] as $value => $label ) : ?>
				<option value="<?php echo esc_attr_layoutbridge_adapter( $value ); ?>"<?php selected( $value, $old_value ); ?>><?php echo $label; ?></option>
			<?php endforeach; ?>
		</select>
