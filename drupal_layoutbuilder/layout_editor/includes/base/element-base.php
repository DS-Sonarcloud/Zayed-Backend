...
abstract class Element_Base extends Controls_Stack {
	 ...
	 * data. For example Drupal widgets use this to store widget names.
	 *
	 * @access private
	 *
	 * @var array
	...
	final public function enqueue_scripts() {
		foreach ( $this->get_script_depends() as $script ) {
			wp_enqueue_script_layoutbridge_adapter( $script );
		}
	}
	...
	final public function enqueue_styles() {
		foreach ( $this->get_style_depends() as $style ) {
			wp_enqueue_style_layoutbridge_adapter( $style );
		}
	}
	...
	final public static function is_edit_buttons_enabled() {
		return get_option_layoutbridge_adapter( 'elementor_edit_buttons' );
	}
	...
	public function get_render_attribute_string( $element ) {
		...
		foreach ( $render_attributes as $attribute_key => $attribute_values ) {
			$attributes[] = sprintf( '%1$s="%2$s"', $attribute_key, esc_attr_layoutbridge_adapter( implode( ' ', $attribute_values ) ) );
		}
	...
	public function print_element() {
		 ...
		 */
		do_action_layoutbridge_adapter( 'elementor/frontend/before_render', $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action_layoutbridge_adapter( "elementor/frontend/{$element_type}/before_render", $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action_layoutbridge_adapter( "elementor/frontend/{$element_type}/after_render", $this );
	}
	...
	public function get_unique_selector() {
		return '.drupal_layoutbuilder-element-' . $this->get_id();
	}
	...
	protected function render_edit_tools() {
		...
		<div class="drupal_layoutbuilder-element-overlay">
			<ul class="drupal_layoutbuilder-editor-element-settings drupal_layoutbuilder-editor-<?php echo $this->get_type(); ?>-settings">
				<?php
	...
	protected function _add_render_attributes() {
		...
		$this->add_render_attribute(
			'_wrapper', 'class', [
				'drupal_layoutbuilder-element',
				'drupal_layoutbuilder-element-' . $id,
			]
		);
		...
		// 	$this->add_render_attribute( '_wrapper', 'class', 'drupal_layoutbuilder-invisible' );
		// }
      ...
      $json_settings = Json::encode($frontend_settings);
      $this->add_render_attribute('_wrapper', 'data-settings', htmlspecialchars($json_settings, ENT_QUOTES, 'UTF-8'));
			// $this->add_render_attribute( '_wrapper', 'data-settings', wp_json_encode_layoutbridge_adapter( $frontend_settings ) );
		}
	}
	...
	private function get_child_type( $element_data ) {
		 ...
		 */
		$child_type = apply_filters_layoutbridge_adapter( 'elementor/element/get_child_type', $child_type, $element_data, $this );
