...
use DrupalLayoutbuilder\Core\DynamicTags\Manager;
...
abstract class Controls_Stack {
		...
		if ( empty( $args['type'] ) || ! in_array( $args['type'], [ Controls_Manager::SECTION, Controls_Manager::WP_WIDGET ], true ) ) {
			...
			if ( null !== $target_section_args ) {
				if ( ! empty( $args['section'] ) || ! empty( $args['tab'] ) ) {
					_doing_it_wrong_layoutbridge_adapter( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), sprintf( 'Cannot redeclare control with `tab` or `section` args inside section "%s".', $id ), '1.0.0' );
				}
			...
			} elseif ( empty( $args['section'] ) && ( ! $options['overwrite'] || is_wp_error_layoutbridge_adapter( Plugin::$instance->controls_manager->get_control_from_stack( $this->get_unique_name(), $id ) ) ) ) {
	...
	final public function get_position_info( array $position ) {
		...
		if (
			...
			_doing_it_wrong_layoutbridge_adapter( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), 'Invalid position arguments. Use `before` / `after` for control or `start` / `end` for section.', '1.7.0' );
	...
	/**
	 * Add missing control keys so render() does not hit PHP 8+ undefined array keys.
	 *
	 * Saved documents (or forked widgets) may omit settings for controls added later.
	 * get_active_settings() only iterates keys present in stored data, so new control
	 * IDs never appear unless we merge defaults here.
	 *
	 * @param array $active_settings Output of get_active_settings().
	 *
	 * @return array
	 */
	protected function fill_missing_active_settings( array $active_settings ) {
		$skip_types = [
			Controls_Manager::SECTION,
			Controls_Manager::TAB,
			Controls_Manager::TABS,
			Controls_Manager::HEADING,
			Controls_Manager::RAW_HTML,
			Controls_Manager::DIVIDER,
			Controls_Manager::WP_WIDGET,
			Controls_Manager::BUTTON,
			Controls_Manager::HIDDEN,
			Controls_Manager::STRUCTURE,
		];

		foreach ( $this->get_controls() as $control_id => $control ) {
			if ( array_key_exists( $control_id, $active_settings ) ) {
				continue;
			}
			...
			if ( Controls_Manager::REPEATER === $type ) {
				$active_settings[ $control_id ] = array_key_exists( 'default', $control ) ? $control['default'] : [];
				continue;
			}

			$active_settings[ $control_id ] = array_key_exists( 'default', $control ) ? $control['default'] : null;
		}
	...
	public function get_settings_for_display( $setting_key = null ) {
		if ( ! $this->parsed_active_settings ) {
			...
			$this->parsed_active_settings = $this->fill_missing_active_settings( $this->parsed_active_settings );
		}
	...
	public function start_controls_section( $section_id, array $args ) {
		 ...
		 * @param array          $args       Section arguments.
		 */
		do_action_layoutbridge_adapter( 'elementor/element/before_section_start', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action_layoutbridge_adapter( "elementor/element/{$section_name}/{$section_id}/before_section_start", $this, $args );
		 ...
		 * @param array          $args       Section arguments.
		 */
		do_action_layoutbridge_adapter( 'elementor/element/after_section_start', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action_layoutbridge_adapter( "elementor/element/{$section_name}/{$section_id}/after_section_start", $this, $args );
	}
	...
	public function end_controls_section() {
		 ...
		 * @param array          $args       Section arguments.
		 */
		do_action_layoutbridge_adapter( 'elementor/element/before_section_end', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action_layoutbridge_adapter( "elementor/element/{$stack_name}/{$section_id}/before_section_end", $this, $args );
		 ...
		 * @param array          $args       Section arguments.
		 */
		do_action_layoutbridge_adapter( 'elementor/element/after_section_end', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action_layoutbridge_adapter( "elementor/element/{$stack_name}/{$section_id}/after_section_end", $this, $args );
	}
	...
	public function print_template() {
		 ...
		 */
		$template_content = apply_filters_layoutbridge_adapter( "elementor/{$element_type}/print_template", $template_content, $this );

		if ( empty( $template_content ) ) {
			return;
		}
		?>
		<script type="text/html" id="tmpl-drupal_layoutbuilder-<?php echo esc_attr_layoutbridge_adapter( $this->get_name() ); ?>-content">
