...
abstract class Widget_Base extends Element_Base
{
	...
	public function __construct($data = [], $args = null)
	{
		...
		if ($is_type_instance) {
			 ...
			 */
			do_action_layoutbridge_adapter("elementor/widget/{$widget_name}/skins_init", $this);
		}
	}
	...
	private function register_skin_control()
	{
		...
		if (! empty($skins)) {
			...
			if ($this->_has_template_content) {
				$skin_options[''] = ___layoutbridge_adapter('Default', 'elementor');
			}
			...
			if (1 >= count($skin_options)) {
				$this->add_control(
					'_skin',
					[
						'label' => ___layoutbridge_adapter('Skin', 'elementor'),
	...
	{
		$widget_label = ___layoutbridge_adapter('Widget', 'elementor');
		...
		if (self::is_edit_buttons_enabled()) {
			$edit_tools += [
				'duplicate' => [
					...
					'title' => sprintf(___layoutbridge_adapter('Duplicate %s', 'elementor'), $widget_label),
					'icon' => 'clone',
				],
				'remove' => [
					/* translators: %s: Widget label */
					'title' => sprintf(___layoutbridge_adapter('Remove %s', 'elementor'), $widget_label),
					'icon' => 'close',
				],
			];
		}
...
?>
		<div class="drupal_layoutbuilder-widget-container">
			<?php
			echo $template_content; // XSS ok.
			?><script type="text/template" id="tmpl-drupal_layoutbuilder-advanced-css-{{{ view.getID() }}}">
			<# 
			if ( settings.advanced_css ) {
				var css = settings.advanced_css;

				// Remove any <style> tags
				css = css.replace(/<\/?style[^>]*>/g, '');

				// Replace selector keyword with unique widget ID
				css = css.replace(/selector/g, '.drupal_layoutbuilder-element-' + view.getID());

				// Inject style dynamically into editor iframe
				var styleId = 'live-advanced-css-' + view.getID();
	...
	protected function parse_text_editor($content)
	{
		...
		$content = apply_filters_layoutbridge_adapter('widget_text', $content, $this->get_settings());

		$content = shortcode_unautop_layoutbridge_adapter($content);
		$content = do_shortcode_layoutbridge_adapter($content);
		$content = wptexturize_layoutbridge_adapter($content);
	...
	{
		return 'drupal_layoutbuilder-widget-' . $this->get_name();
	}
	...
	public function render_content()
	{
		 ...
		 * @param Widget_Base $this The current widget.
		 */
		do_action_layoutbridge_adapter('elementor/widget/before_render_content', $this);
	...
	?>
		<div class="drupal_layoutbuilder-widget-container">
			 ...
			 */
			$widget_content = apply_filters_layoutbridge_adapter('elementor/widget/render_content', $widget_content, $this);

			echo $widget_content; // XSS ok.
			$settings = $this->get_settings_for_display();

			if (!empty($settings['advanced_css'])) {
				$custom_css = $settings['advanced_css'];
				$custom_css = preg_replace('/<\/?style[^>]*>/', '', $custom_css);
				$custom_css = str_replace('selector', '.drupal_layoutbuilder-element-' . $this->get_id(), $custom_css);
	 ...
	 * Drupal saves data. This method is used to save generated HTML to the
	 * database as plain content the Drupal way.
	 *
	 * When rendering plain content, it allows other Drupal plugins to
