...
class Widget_Google_Maps extends Widget_Base {
	...
	public function get_title() {
		return ___layoutbridge_adapter( 'Google Maps', 'elementor' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_map',
			[
				'label' => ___layoutbridge_adapter( 'Map', 'elementor' ),
			]
		);

		$default_address = ___layoutbridge_adapter( 'London Eye, London, United Kingdom', 'elementor' );
		$this->add_control(
			'address',
			[
				'label' => ___layoutbridge_adapter( 'Address', 'elementor' ),
		...
		$this->add_control(
			'zoom',
			[
				'label' => ___layoutbridge_adapter( 'Zoom', 'elementor' ),
		...
		$this->add_control(
			'prevent_scroll',
			[
				'label' => ___layoutbridge_adapter( 'Prevent Scroll', 'elementor' ),
	...
	protected function render() {
		...
		if ( 0 === absint_layoutbridge_adapter( $settings['zoom']['size'] ) ) {
			$settings['zoom']['size'] = 10;
		}

		printf(
			'<div class="drupal_layoutbuilder-custom-embed"><iframe frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?q=%s&amp;t=m&amp;z=%d&amp;output=embed&amp;iwloc=near" aria-label="%s"></iframe></div>',
			rawurlencode( $settings['address'] ),
			absint_layoutbridge_adapter( $settings['zoom']['size'] ),
			esc_attr_layoutbridge_adapter( $settings['address'] )
		);
	}
