...
class Autoloader {
	...
	private static function load_class( $relative_class_name ) {
		if ( isset( self::$classes_map[ $relative_class_name ] ) ) {
			$filename = DRUPAL_LAYOUTBUILDER_PATH . '/' . self::$classes_map[ $relative_class_name ];
		} else {
			$filename = strtolower(
				preg_replace(
					[ '/([a-z])([A-Z])/', '/_/', '/\\\/' ],
			...
			$filename = DRUPAL_LAYOUTBUILDER_PATH . $filename . '.php';
		}
	...
	private static function autoload( $class ) {
		...
		if ( $has_class_alias ) {
			...
			preg_match( '/^[0-9]+\.[0-9]+/', DRUPAL_LAYOUTBUILDER_VERSION, $current_version_as_float );
