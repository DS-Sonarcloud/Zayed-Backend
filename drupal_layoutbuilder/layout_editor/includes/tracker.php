...
class Tracker {
	...
	public static function init() {
		add_action_layoutbridge_adapter( 'elementor/tracker/send_event', [ __CLASS__, 'send_tracking_data' ] );
		add_action_layoutbridge_adapter( 'admin_init', [ __CLASS__, 'handle_tracker_actions' ] );
		add_action_layoutbridge_adapter( 'admin_notices', [ __CLASS__, 'admin_notices' ] );
	}
	...
	public static function check_for_settings_optin( $new_value ) {
		$old_value = get_option_layoutbridge_adapter( 'elementor_allow_tracking', 'no' );
	...
	public static function send_tracking_data( $override = false ) {
		 ...
		 */
		$override = apply_filters_layoutbridge_adapter( 'elementor/tracker/send_override', $override );
		...
		if ( ! $override ) {
			 ...
			 */
			$last_send_interval = apply_filters_layoutbridge_adapter( 'elementor/tracker/last_send_interval', $last_send_interval );
		...
		update_option_layoutbridge_adapter( 'elementor_tracker_last_send', time() );

		// Send here..
		$params = [
			'system' => self::get_system_reports_data(),
			'site_lang' => get_bloginfo_layoutbridge_adapter( 'language' ),
			'email' => get_option_layoutbridge_adapter( 'admin_email' ),
			'usages' => [
		 ...
		 */
		$params = apply_filters_layoutbridge_adapter( 'elementor/tracker/send_tracking_data_params', $params );

		add_filter_layoutbridge_adapter( 'https_ssl_verify', '__return_false' );
		...
		wp_safe_remote_post(
			...
			[
				...
				// 'sslverify' => false,
				'body' => [
					'data' => wp_json_encode_layoutbridge_adapter( $params ),
				],
			]
		);
	}
	...
	public static function is_allow_track() {
		return 'yes' === get_option_layoutbridge_adapter( 'elementor_allow_tracking', 'no' );
	}
	...
	public static function handle_tracker_actions() {
		...
		if ( 'opt_into' === $_GET['elementor_tracker'] ) {
			check_admin_referer_layoutbridge_adapter( 'opt_into' );

			update_option_layoutbridge_adapter( 'elementor_allow_tracking', 'yes' );
			self::send_tracking_data( true );
		}

		if ( 'opt_out' === $_GET['elementor_tracker'] ) {
			check_admin_referer_layoutbridge_adapter( 'opt_out' );

			update_option_layoutbridge_adapter( 'elementor_allow_tracking', 'no' );
			update_option_layoutbridge_adapter( 'elementor_tracker_notice', '1' );
		}

		wp_redirect_layoutbridge_adapter( remove_query_arg( 'elementor_tracker' ) );
		exit;
	}
	 ...
	 * Admin notices.
	 *
	 * Add Elementor notices to Drupal admin screen to show tracker notice.
	 *
	...
	public static function admin_notices() {
		...
		if ( '1' === get_option_layoutbridge_adapter( 'elementor_tracker_notice' ) ) {
			return;
		}
		...
		$optin_url = wp_nonce_url( add_query_arg_layoutbridge_adapter( 'elementor_tracker', 'opt_into' ), 'opt_into' );
		$optout_url = wp_nonce_url( add_query_arg_layoutbridge_adapter( 'elementor_tracker', 'opt_out' ), 'opt_out' );

		$tracker_description_text = ___layoutbridge_adapter( 'Love using Elementor? Become a super contributor by opting in to our anonymous plugin data collection and to our updates. We guarantee no sensitive data is collected.', 'elementor' );
		 ...
		 */
		$tracker_description_text = apply_filters_layoutbridge_adapter( 'elementor/tracker/admin_description_text', $tracker_description_text );
		?>
		<div class="notice updated drupal_layoutbuilder-message">
			<div class="drupal_layoutbuilder-message-inner">
				...
				<div class="drupal_layoutbuilder-message-content">
					<p><?php echo esc_html_layoutbridge_adapter( $tracker_description_text ); ?> <a href="https://go.elementor.com/usage-data-tracking/" target="_blank"><?php echo ___layoutbridge_adapter( 'Learn more.', 'elementor' ); ?></a></p>
					<p class="drupal_layoutbuilder-message-actions">
						<a href="<?php echo $optin_url; ?>" class="button button-primary"><?php echo ___layoutbridge_adapter( 'Sure! I\'d love to help', 'elementor' ); ?></a>&nbsp;<a href="<?php echo $optout_url; ?>" class="button-secondary"><?php echo ___layoutbridge_adapter( 'No thanks', 'elementor' ); ?></a>
					</p>
				</div>
			</div>
		</div>
	...
	private static function get_installed_time() {
		$installed_time = get_option_layoutbridge_adapter( '_elementor_installed_time' );
		if ( ! $installed_time ) {
			$installed_time = time();
			update_option_layoutbridge_adapter( '_elementor_installed_time', $installed_time );
		}
	...
	private static function get_last_send_time() {
		$last_send_time = get_option_layoutbridge_adapter( 'elementor_tracker_last_send', false );
		 ...
		 */
		$last_send_time = apply_filters_layoutbridge_adapter( 'elementor/tracker/last_send_time', $last_send_time );
