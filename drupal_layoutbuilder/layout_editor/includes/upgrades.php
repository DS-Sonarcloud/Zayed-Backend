...
class Upgrades {
	 ...
	 * Add actions.
	 *
	 * Hook into Drupal actions and launch Elementor upgrades.
	 *
	 * @static
	 * @since 1.0.0
	 * @access public
	...
	public static function add_actions() {
		add_action_layoutbridge_adapter( 'init', [ __CLASS__, 'init' ], 20 );
	}
	...
	public static function init() {
		$elementor_version = get_option_layoutbridge_adapter( 'elementor_version' );

		// Normal init.
		if ( DRUPAL_LAYOUTBUILDER_VERSION === $elementor_version ) {
			return;
		}
		...
		update_option_layoutbridge_adapter( 'elementor_version', DRUPAL_LAYOUTBUILDER_VERSION );
	}
	...
	private static function check_upgrades( $elementor_version ) {
		...
		$elementor_upgrades = get_option_layoutbridge_adapter( 'elementor_upgrades', [] );
		...
		foreach ( $upgrades as $version => $function ) {
			if ( version_compare( $elementor_version, $version, '<' ) && ! isset( $elementor_upgrades[ $version ] ) ) {
				...
				update_option_layoutbridge_adapter( 'elementor_upgrades', $elementor_upgrades );
			}
		}
	}
	...
	private static function upgrade_v200() {
		...
		foreach ( $posts as $post ) {
			wp_update_post( [
				'ID' => $post->ID,
				'post_title' => get_the_title_layoutbridge_adapter( $post->post_parent ),
			] );
		}
	}
	...
	private static function upgrade_v201() {
		...
		foreach ( $posts as $post ) {
			$parent = get_post_layoutbridge_adapter( $post->post_parent );
	...
	private static function upgrade_v210() {
		...
		foreach ( $post_ids as $post_id ) {
			...
			$json_value = wp_slash( wp_json_encode_layoutbridge_adapter( $data ) );
