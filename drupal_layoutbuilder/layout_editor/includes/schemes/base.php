...
abstract class Scheme_Base implements Scheme_Interface {
	...
	public function get_scheme_value() {
		$scheme_value = get_option_layoutbridge_adapter( 'elementor_scheme_' . static::get_type() );

		if ( ! $scheme_value ) {
			$scheme_value = $this->get_default_scheme();

			update_option_layoutbridge_adapter( 'elementor_scheme_' . static::get_type(), $scheme_value );
		}
	...
	public function save_scheme( array $posted ) {
		...
		update_option_layoutbridge_adapter( 'elementor_scheme_' . static::get_type(), array_replace( $scheme_value, array_intersect_key( $posted, $scheme_value ) ) );

		update_option_layoutbridge_adapter( self::LAST_UPDATED_META, time() );
	}
	...
	final public function print_template() {
		?>
		<script type="text/template" id="tmpl-drupal_layoutbuilder-panel-schemes-<?php echo static::get_type(); ?>">
			<div class="drupal_layoutbuilder-panel-scheme-buttons">
				<div class="drupal_layoutbuilder-panel-scheme-button-wrapper drupal_layoutbuilder-panel-scheme-reset">
					<button class="drupal_layoutbuilder-button">
						<i class="fa fa-undo" aria-hidden="true"></i>
						<?php echo ___layoutbridge_adapter( 'Reset', 'elementor' ); ?>
					</button>
				</div>
				...
				<div class="drupal_layoutbuilder-panel-scheme-button-wrapper drupal_layoutbuilder-panel-scheme-save">
					<button class="drupal_layoutbuilder-button drupal_layoutbuilder-button-success" disabled><?php echo ___layoutbridge_adapter( 'Apply', 'elementor' ); ?></button>
				</div>
			</div>
