...
class Compatibility {
	...
	public static function register_actions() {
		add_action_layoutbridge_adapter( 'init', [ __CLASS__, 'init' ] );

		self::polylang_compatibility();

		if ( is_admin_layoutbridge_adapter() || defined( 'WP_LOAD_IMPORTERS' ) ) {
			add_filter_layoutbridge_adapter( 'wp_import_post_meta', [ __CLASS__, 'on_wp_import_post_meta' ] );
			add_filter_layoutbridge_adapter( 'wxr_importer.pre_process.post_meta', [ __CLASS__, 'on_wxr_importer_pre_process_post_meta' ] );
		}
	}
	...
	public static function add_new_button_to_gutenberg() {
		...
		<script type="text/javascript">
			document.addEventListener( 'DOMContentLoaded', function() {
				...
				var url = '<?php echo esc_url_layoutbridge_adapter( Utils::get_create_new_post_url( $typenow ) ); ?>';
	...
	public static function init() {
		...
		if ( defined( 'NGG_PLUGIN_VERSION' ) ) {
			add_filter_layoutbridge_adapter( 'elementor/utils/get_edit_link', function( $edit_link ) {
				return add_query_arg_layoutbridge_adapter( 'display_gallery_iframe', '', $edit_link );
			} );
		}

		// Hack for Ninja Forms.
		if ( class_exists( '\Ninja_Forms' ) && class_exists( '\NF_Display_Render' ) ) {
			add_action_layoutbridge_adapter( 'elementor/preview/enqueue_styles', function() {
				ob_start();
		...
		add_filter_layoutbridge_adapter( 'wpseo_sitemaps_supported_post_types', function( $post_types ) {
			unset( $post_types[ Source_Local::CPT ] );

			return $post_types;
		} );

		add_filter_layoutbridge_adapter( 'wpseo_sitemap_exclude_post_type', function( $retval, $post_type ) {
		...
		add_filter_layoutbridge_adapter( 'autoptimize_filter_noptimize', function( $retval ) {
		...
		add_filter_layoutbridge_adapter( 'woocommerce_product_tabs', function( $tabs ) {
			if ( ! isset( $tabs['description'] ) && Plugin::$instance->preview->is_preview_mode() ) {
				$post = get_post_layoutbridge_adapter();
				if ( empty( $post->post_content ) ) {
					$tabs['description'] = [
						'title' => ___layoutbridge_adapter( 'Description', 'elementor' ),
						'priority' => 10,
		...
		if ( function_exists( 'WC' ) ) {
			add_action_layoutbridge_adapter( 'elementor/editor/before_enqueue_scripts', function() {
				remove_action_layoutbridge_adapter( 'woocommerce_shortcode_before_product_cat_loop', 'wc_print_notices' );
				remove_action_layoutbridge_adapter( 'woocommerce_before_shop_loop', 'wc_print_notices' );
				remove_action_layoutbridge_adapter( 'woocommerce_before_single_product', 'wc_print_notices' );
			} );
		}
		...
		if ( class_exists( 'Grunion_Editor_View' ) ) {
			add_action_layoutbridge_adapter( 'elementor/editor/before_enqueue_scripts', function() {
				remove_action_layoutbridge_adapter( 'media_buttons', 'grunion_media_button', 999 );
				remove_action_layoutbridge_adapter( 'admin_enqueue_scripts', 'grunion_enable_spam_recheck' );

				remove_action_layoutbridge_adapter( 'admin_notices', [ 'Grunion_Editor_View', 'handle_editor_view_js' ] );
				remove_action_layoutbridge_adapter( 'admin_head', [ 'Grunion_Editor_View', 'admin_head' ] );
			} );
		}
		...
		if ( class_exists( 'PUM_Admin_Shortcode_UI' ) ) {
			add_action_layoutbridge_adapter( 'elementor/editor/before_enqueue_scripts', function() {
				...
				remove_action_layoutbridge_adapter( 'print_media_templates', [ $pum_admin_instance, 'print_media_templates' ] );
				remove_action_layoutbridge_adapter( 'admin_print_footer_scripts', [ $pum_admin_instance, 'admin_print_footer_scripts' ], 100 );
				remove_action_layoutbridge_adapter( 'wp_ajax_pum_do_shortcode', [ $pum_admin_instance, 'wp_ajax_pum_do_shortcode' ] );

				remove_action_layoutbridge_adapter( 'admin_enqueue_scripts', [ $pum_admin_instance, 'admin_enqueue_scripts' ] );

				remove_filter_layoutbridge_adapter( 'pum_admin_var', [ $pum_admin_instance, 'pum_admin_var' ] );
			} );
		}

		// Fix Preview URL for https://premium.wpmudev.org/project/domain-mapping/ plugin
		if ( class_exists( 'domain_map' ) ) {
			add_filter_layoutbridge_adapter( 'elementor/document/urls/preview', function( $preview_url ) {
				if ( wp_parse_url( $preview_url, PHP_URL_HOST ) !== $_SERVER['HTTP_HOST'] ) {
					$preview_url = \domain_map::utils()->unswap_url( $preview_url );
					$preview_url = add_query_arg_layoutbridge_adapter( [
		...
		if ( function_exists( 'gutenberg_init' ) ) {
			add_action_layoutbridge_adapter( 'admin_print_scripts-edit.php', [ __CLASS__, 'add_new_button_to_gutenberg' ], 11 );
		}
	}
	...
	private static function polylang_compatibility() {
		...
		if ( isset( $_REQUEST['action'] ) && 0 === strpos( $_REQUEST['action'], 'elementor' ) ) {
			add_action_layoutbridge_adapter( 'set_current_user', function() {
				global $current_user;
			...
			add_action_layoutbridge_adapter( 'pll_pre_init', function( $polylang ) {
		...
		add_filter_layoutbridge_adapter( 'pll_copy_post_metas', [ __CLASS__, 'save_polylang_meta' ], 10, 4 );
	}
