...
use DrupalLayoutbuilder\TemplateLibrary\Source_Local;
...
class Editor {
	 ...
	 */
	const EDITING_NONCE_KEY = 'drupal_layoutbuilder-editing';
	...
	public function init( $die = true ) {
		...
		$this->_post_id = absint_layoutbridge_adapter( $_REQUEST['post'] );
		...
		@header( 'Content-Type: ' . get_option_layoutbridge_adapter( 'html_type' ) . '; charset=' . get_option_layoutbridge_adapter( 'blog_charset' ) );

		// Use requested id and not the global in order to avoid conflicts with plugins that changes the global post.
		query_posts_layoutbridge_adapter( [
			'p' => $this->_post_id,
			'post_type' => get_post_type_layoutbridge_adapter( $this->_post_id ),
		] );

		Plugin::$instance->db->switch_to_post( $this->_post_id );

		add_filter_layoutbridge_adapter( 'show_admin_bar', '__return_false' );

		// Remove all Drupal actions
		...
		// Handle `wp_head`
		add_action_layoutbridge_adapter( 'wp_head', 'wp_enqueue_scripts', 1 );
		add_action_layoutbridge_adapter( 'wp_head', 'wp_print_styles', 8 );
		add_action_layoutbridge_adapter( 'wp_head', 'wp_print_head_scripts', 9 );
		add_action_layoutbridge_adapter( 'wp_head', 'wp_site_icon' );
		add_action_layoutbridge_adapter( 'wp_head', [ $this, 'editor_head_trigger' ], 30 );

		// Handle `wp_footer`
		add_action_layoutbridge_adapter( 'wp_footer', 'wp_print_footer_scripts', 20 );
		add_action_layoutbridge_adapter( 'wp_footer', 'wp_auth_check_html', 30 );
		add_action_layoutbridge_adapter( 'wp_footer', [ $this, 'wp_footer' ] );

		// Handle `wp_enqueue_scripts`
		remove_all_actions( 'wp_enqueue_scripts' );

		add_action_layoutbridge_adapter( 'wp_enqueue_scripts', [ $this, 'enqueue_scripts' ], 999999 );
		add_action_layoutbridge_adapter( 'wp_enqueue_scripts', [ $this, 'enqueue_styles' ], 999999 );
		...
		add_filter_layoutbridge_adapter( 'heartbeat_settings', function( $settings ) {
	...
	public function redirect_to_new_url() {
		...
		wp_redirect_layoutbridge_adapter( Utils::get_edit_link( $post_id ) );
		die;
	}
	...
	public function enqueue_scripts() {
		remove_action_layoutbridge_adapter( 'wp_enqueue_scripts', [ $this, __FUNCTION__ ], 999999 );

		// Set the global data like $post, $authordata and etc
		setup_postdata_layoutbridge_adapter( $this->_post_id );
		...
		$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG || defined( 'DRUPAL_LAYOUTBUILDER_TESTS' ) && DRUPAL_LAYOUTBUILDER_TESTS ) ? '' : '.min';

		// Hack for waypoint with editor mode.
		wp_register_script_layoutbridge_adapter(
			'drupal_layoutbuilder-waypoints',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/waypoints/waypoints-for-editor.js',
			[
				'jquery',
			],
			'4.0.2',
		...
		wp_register_script_layoutbridge_adapter(
			'backbone-radio',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/backbone/backbone.radio' . $suffix . '.js',
			[
				'backbone',
			],
			'1.0.4',
		...
		wp_register_script_layoutbridge_adapter(
			'perfect-scrollbar',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/perfect-scrollbar/perfect-scrollbar.jquery' . $suffix . '.js',
			[
				'jquery',
			],
			'0.6.12',
		...
		wp_register_script_layoutbridge_adapter(
			'jquery-easing',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/jquery-easing/jquery-easing' . $suffix . '.js',
			[
				'jquery',
			],
			'1.3.2',
		...
		wp_register_script_layoutbridge_adapter(
			'nprogress',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/nprogress/nprogress' . $suffix . '.js',
			[],
			'0.2.0',
			true
		);
		...
		wp_register_script_layoutbridge_adapter(
			'tipsy',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/tipsy/tipsy' . $suffix . '.js',
			[
				'jquery',
			],
			'1.0.0',
		...
		wp_register_script_layoutbridge_adapter(
			'jquery-drupal_layoutbuilder-select2',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/e-select2/js/e-select2.full' . $suffix . '.js',
			[
				'jquery',
			],
			'4.0.6-rc.1',
		...
		wp_register_script_layoutbridge_adapter(
			'flatpickr',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/flatpickr/flatpickr' . $suffix . '.js',
			[
				'jquery',
			],
			'1.12.0',
		...
		wp_register_script_layoutbridge_adapter(
			'jquery-hover-intent',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/jquery-hover-intent/jquery-hover-intent' . $suffix . '.js',
			[],
			'1.0.0',
			true
		);
		...
		wp_register_script_layoutbridge_adapter(
			'drupal_layoutbuilder-editor',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'js/editor' . $suffix . '.js',
			[
				'wp-auth-check',
				'jquery-ui-sortable',
				'jquery-ui-resizable',
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/editor/before_enqueue_scripts' );
		...
		wp_enqueue_script_layoutbridge_adapter( 'drupal_layoutbuilder-editor' );
		...
		$page_title_selector = get_option_layoutbridge_adapter( 'elementor_page_title_selector' );

		if ( empty( $page_title_selector ) ) {
			$page_title_selector = 'h1.entry-title';
		}

		$post_type_object = get_post_type_object_layoutbridge_adapter( $document->get_main_post()->post_type );
		$current_user_can_publish = current_user_can( $post_type_object->cap->publish_posts );

		$config = [
			'ajaxurl' => admin_url_layoutbridge_adapter( 'admin-ajax.php' ),
			'home_url' => home_url(),
			'nonce' => $this->create_nonce( get_post_type_layoutbridge_adapter() ),
			'data' => $editor_data,
			...
			'assets_url' => DRUPAL_LAYOUTBUILDER_ASSETS_URL,
			...
			'is_rtl' => is_rtl_layoutbridge_adapter(),
			'locale' => get_locale(),
			'rich_editing_enabled' => filter_var( get_user_meta_layoutbridge_adapter( get_current_user_id_layoutbridge_adapter(), 'rich_editing', true ), FILTER_VALIDATE_BOOLEAN ),
		 ...
		 */
		$localized_settings = apply_filters_layoutbridge_adapter( 'elementor/editor/localize_settings', $localized_settings, $this->_post_id );
		...
		echo '<script>' . PHP_EOL;
		echo '/* <![CDATA[ */' . PHP_EOL;
		$config_json = wp_json_encode_layoutbridge_adapter( $config );
		unset( $config );

		if ( get_option_layoutbridge_adapter( 'elementor_editor_break_lines' ) ) {
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/editor/after_enqueue_scripts' );
	}
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/editor/before_enqueue_styles' );
		...
		wp_register_style(
			'font-awesome',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/font-awesome/css/font-awesome' . $suffix . '.css',
			[],
			'4.7.0'
		);

		wp_register_style(
			'drupal_layoutbuilder-select2',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/e-select2/css/e-select2' . $suffix . '.css',
			[],
			'4.0.6-rc.1'
		);
		...
		wp_register_style(
			'flatpickr',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'lib/flatpickr/flatpickr' . $suffix . '.css',
			[],
			'1.12.0'
		);
		...
		wp_register_style(
			'drupal_layoutbuilder-editor',
			DRUPAL_LAYOUTBUILDER_ASSETS_URL . 'css/editor' . $direction_suffix . $suffix . '.css',
			[
				'font-awesome',
				'drupal_layoutbuilder-select2',
				'drupal_layoutbuilder-icons',
		...
		wp_enqueue_style_layoutbridge_adapter( 'drupal_layoutbuilder-editor' );

		if ( Responsive::has_custom_breakpoints() ) {
			$breakpoints = Responsive::get_breakpoints();

			wp_add_inline_style( 'drupal_layoutbuilder-editor', '.drupal_layoutbuilder-device-tablet #drupal_layoutbuilder-preview-responsive-wrapper { width: ' . $breakpoints['md'] . 'px; }' );
		}
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/editor/after_enqueue_styles' );
	}

	/**
	 * Get Drupal editor config.
	 *
	 * Config the default Drupal editor with custom settings for Elementor use.
	 *
	 * @since 1.9.0
	 * @access private
	 */
	private function get_wp_editor_config() {
		...
		wp_editor(
			...
			'elementorwpeditor',
			[
				'editor_class' => 'drupal_layoutbuilder-wp-editor',
				'editor_height' => 250,
		...
		remove_action_layoutbridge_adapter( 'admin_print_footer_scripts', [ '_WP_Editors', 'editor_js' ], 50 );
		remove_action_layoutbridge_adapter( 'admin_print_footer_scripts', [ '_WP_Editors', 'print_default_editor_scripts' ], 45 );
	...
	public function editor_head_trigger() {
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/editor/wp_head' );
	}
	...
	public function wp_footer() {
		 ...
		 * @since 1.0.0
		 */
		do_action_layoutbridge_adapter( 'elementor/editor/footer' );
	}
	...
	public function __construct() {
		add_action_layoutbridge_adapter( 'admin_action_elementor', [ $this, 'init' ] );
		add_action_layoutbridge_adapter( 'template_redirect', [ $this, 'redirect_to_new_url' ] );

		// Handle autocomplete feature for URL control.
		add_filter_layoutbridge_adapter( 'wp_link_query_args', [ $this, 'filter_wp_link_query_args' ] );
		add_filter_layoutbridge_adapter( 'wp_link_query', [ $this, 'filter_wp_link_query' ] );
	}
	...
	public function filter_wp_link_query( $results ) {
		if ( isset( $_POST['editor'] ) && 'elementor' === $_POST['editor'] ) {
			$post_type_object = get_post_type_object_layoutbridge_adapter( 'post' );
			$post_label = $post_type_object->labels->singular_name;

			foreach ( $results as & $result ) {
				if ( 'post' === get_post_type_layoutbridge_adapter( $result['ID'] ) ) {
	...
	public function create_nonce( $post_type ) {
		$post_type_object = get_post_type_object_layoutbridge_adapter( $post_type );
		...
		return wp_create_nonce_layoutbridge_adapter( self::EDITING_NONCE_KEY );
	}
	...
	public function verify_nonce( $nonce ) {
		return wp_verify_nonce_layoutbridge_adapter( $nonce, self::EDITING_NONCE_KEY );
	}
	...
	public function verify_ajax_nonce() {
		if ( ! $this->verify_request_nonce() ) {
			wp_send_json_error_layoutbridge_adapter( new \WP_Error( 'token_expired', 'Nonce token expired.' ) );
		}
	}
