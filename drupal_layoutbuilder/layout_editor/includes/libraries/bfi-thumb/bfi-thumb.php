...
if ( ! defined( 'BFITHUMB_UPLOAD_DIR' ) ) {
	define( 'BFITHUMB_UPLOAD_DIR', 'layout_editor/thumbs' );
}
 ...
 */
add_filter_layoutbridge_adapter( 'wp_image_editors', 'bfi_wp_image_editor' );
...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
	...
	class BFI_Thumb_1_3 {
		...
		public static function thumb( $url, $params = array(), $single = true ) {
			...
			$upload_info = wp_upload_dir_layoutbridge_adapter();
			...
			if ( ! is_dir( $upload_dir ) ) {
				wp_mkdir_p_layoutbridge_adapter( $upload_dir );
			}
			...
			if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
				...
				if ( is_wp_error_layoutbridge_adapter( $editor ) ) {
					return false;
				}
				...
				if ( $crop_only === false ) {
					if ( ( isset( $width ) && $width ) || ( isset( $height ) && $height ) ) {
						if ( is_wp_error_layoutbridge_adapter( $editor->resize( isset( $width ) ? $width : null, isset( $height ) ? $height : null, isset( $crop ) ? $crop : false ) ) ) {
							return false;
						}
					}
				} else {
					if ( is_wp_error_layoutbridge_adapter( $editor->crop( $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h ) ) ) {
						return false;
					}
				}
				...
				if ( isset( $negate ) ) {
					if ( $negate ) {
						if ( is_wp_error_layoutbridge_adapter( $editor->negate() ) ) {
							return false;
						}
					}
				}
				...
				if ( isset( $opacity ) ) {
					if ( is_wp_error_layoutbridge_adapter( $editor->opacity( $opacity ) ) ) {
						return false;
					}
				}
				...
				if ( isset( $grayscale ) ) {
					if ( $grayscale ) {
						if ( is_wp_error_layoutbridge_adapter( $editor->grayscale() ) ) {
							return false;
						}
					}
				}
				...
				if ( isset( $color ) ) {
					if ( is_wp_error_layoutbridge_adapter( $editor->colorize( $color ) ) ) {
						return false;
					}
				}
...
if ( ! is_admin_layoutbridge_adapter() ) {
	add_filter_layoutbridge_adapter( 'image_resize_dimensions', 'bfi_image_resize_dimensions', 10, 5 );
}
...
add_filter_layoutbridge_adapter( 'image_downsize', 'bfi_image_downsize', 1, 3 );
