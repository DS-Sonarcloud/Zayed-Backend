...
use DrupalLayoutbuilder\Core\Settings\Page\Manager as PageSettingsManager;
...
class Source_Local extends Source_Base {
	...
	public function get_items( $args = [] ) {
		...
		if ( $templates_query->have_posts_layoutbridge_adapter() ) {
	...
	public function save_item( $template_data ) {
		...
		$template_id = wp_insert_post( [
			'post_title' => ! empty( $template_data['title'] ) ? $template_data['title'] : ___layoutbridge_adapter( '(no title)', 'elementor' ),
	...
	public function get_item( $template_id ) {
		$post = get_post_layoutbridge_adapter( $template_id );
		...
		$data = [
			...
			'date' => $date,
			'human_date' => date_i18n( get_option_layoutbridge_adapter( 'date_format' ), $date ),
	...
	public function delete_template( $template_id ) {
		if ( ! current_user_can( $this->post_type_object->cap->delete_post, $template_id ) ) {
			return new \WP_Error( 'template_error', ___layoutbridge_adapter( 'Access denied.', 'elementor' ) );
		}
	 ...
	 * @param int $template_id The template ID.
	 *
	 * @return array|\WP_Error Drupal error if template export failed.
	 */
	...
	private function get_export_link( $template_id ) {
		return add_query_arg_layoutbridge_adapter(
			...
			admin_url_layoutbridge_adapter( 'admin-ajax.php' )
		);
	}
	...
	private function prepare_template_export( $template_id ) {
		...
		$export_data = [
			...
			'title' => get_the_title_layoutbridge_adapter( $template_id ),
