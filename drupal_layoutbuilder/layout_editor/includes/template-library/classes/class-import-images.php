<?php
namespace DrupalLayoutbuilder\TemplateLibrary\Classes;
...
class Import_Images {
	 ...
	 * Import image.
	 *
	 * Import a single image from a remote server, upload the image Drupal
	...
	public function import( $attachment ) {
		...
		$file_content = wp_remote_retrieve_body_layoutbridge_adapter( wp_safe_remote_get( $attachment['url'] ) );
		...
		if ( $info ) {
			...
			return $attachment;
			// return new \WP_Error( 'attachment_processing_error', ___layoutbridge_adapter( 'Invalid file type.', 'elementor' ) );
		}
		...
		update_post_meta_layoutbridge_adapter( $post_id, '_elementor_source_image_hash', $this->get_hash_image( $attachment['url'] ) );
