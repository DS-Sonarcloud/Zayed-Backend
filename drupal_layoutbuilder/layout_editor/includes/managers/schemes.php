...
class Schemes_Manager {
	...
	public static function get_enabled_schemes() {
		if ( null === self::$_enabled_schemes ) {
			...
			foreach ( self::$_schemes_types as $schemes_type => $scheme_class ) {
				if ( 'yes' === get_option_layoutbridge_adapter( 'elementor_disable_' . $schemes_type . '_schemes' ) ) {
					continue;
				}
			 ...
			 */
			$enabled_schemes = apply_filters_layoutbridge_adapter( 'elementor/schemes/enabled_schemes', $enabled_schemes );
	...
	public function __construct() {
		...
		add_action_layoutbridge_adapter( 'wp_ajax_elementor_apply_scheme', [ $this, 'ajax_apply_scheme' ] );
	}
}
