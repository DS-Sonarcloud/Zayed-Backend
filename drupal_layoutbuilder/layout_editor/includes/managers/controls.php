...
class Controls_Manager {
	...
	/**
	 * Drupal Select control.
	 */
	//const DRUPAL_SELECT = 'drupal_select';
	...
	/**
	 * Drupal Media control.
	 */
	const MEDIA_DRUPAL = 'drupal_media';
	...
	const GALLERY_DRUPAL = 'drupal_gallery';
	/**
	 * Structure control.
	 */
	const DRUPAL_CUSTOM_CSS = 'drupal_custom_css';
	/**
	 * Structure control.
	 */
	...
	private static function init_tabs() {
		self::$tabs = [
			self::TAB_CONTENT => ___layoutbridge_adapter( 'Content', 'elementor' ),
			self::TAB_STYLE => ___layoutbridge_adapter( 'Style', 'elementor' ),
			self::TAB_ADVANCED => ___layoutbridge_adapter( 'Advanced', 'elementor' ),
			self::TAB_RESPONSIVE => ___layoutbridge_adapter( 'Responsive', 'elementor' ),
			self::TAB_LAYOUT => ___layoutbridge_adapter( 'Layout', 'elementor' ),
			self::TAB_SETTINGS => ___layoutbridge_adapter( 'Settings', 'elementor' ),
		];
	}
	...
	private function register_controls() {
		...
		foreach ( $available_controls as $control_id ) {
			...
			$control_filename = DRUPAL_LAYOUTBUILDER_PATH . "includes/controls/{$control_filename}.php";
		 ...
		 */
		do_action_layoutbridge_adapter( 'elementor/controls/controls_registered', $this );
	}
	...
	public function add_control_to_stack( Controls_Stack $element, $control_id, $control_data, $options = [] ) {
		...
		if ( ! $control_type_instance ) {
			_doing_it_wrong_layoutbridge_adapter( sprintf( '%1$s::%2$s', __CLASS__, __FUNCTION__ ), sprintf( 'Control type "%s" not found.', $control_data['type'] ), '1.0.0' );
			return false;
		}
		...
		if ( ! $options['overwrite'] && isset( $this->stacks[ $stack_id ]['controls'][ $control_id ] ) ) {
			_doing_it_wrong_layoutbridge_adapter( sprintf( '%1$s::%2$s', __CLASS__, __FUNCTION__ ), sprintf( 'Cannot redeclare control with same name "%s".', $control_id ), '1.0.0' );
	...
	public function update_control_in_stack( Controls_Stack $element, $control_id, $control_data, array $options = [] ) {
		...
		if ( is_wp_error_layoutbridge_adapter( $old_control_data ) ) {
			return false;
		}
    ...
    $element->add_control(
			...
			[
				...
				'description' => t( 'Use custom CSS to style your content or add the <style> "selector" </style> to target specific elements. <style> is prefixed </style>'),
				'language' => 'css',
