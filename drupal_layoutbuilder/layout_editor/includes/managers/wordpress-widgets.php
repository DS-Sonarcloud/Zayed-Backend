...
/**
 * Elementor Drupal widgets manager.
 *
 * Elementor Drupal widgets manager handler class is responsible for
...
class WordPress_Widgets_Manager {
	...
	/**
	 * Drupal widgets manager constructor.
	 *
	 * Initializing the Drupal widgets manager in Elementor editor.
	 *
	 * @since 1.5.0
	 * @access public
	 */
	public function __construct() {
		if ( version_compare( get_bloginfo_layoutbridge_adapter( 'version' ), '4.8', '<' ) ) {
			return;
		}

		add_action_layoutbridge_adapter( 'elementor/editor/before_enqueue_scripts', [ $this, 'before_enqueue_scripts' ] );
		add_action_layoutbridge_adapter( 'elementor/editor/footer', [ $this, 'footer' ] );
	}

	/**
	 * Before enqueue scripts.
	 *
	 * Prints custom scripts required to run Drupal widgets in Elementor
	 * editor.
	 *
	...
	public function before_enqueue_scripts() {
		...
		wp_enqueue_style_layoutbridge_adapter( 'widgets' );
		wp_enqueue_style_layoutbridge_adapter( 'media-views' );
		// End TODO.

		// Don't enqueue `code-editor` for WP Custom HTML widget.
		wp_get_current_user_layoutbridge_adapter()->syntax_highlighting = 'false';

		/** This action is documented in wp-admin/admin-header.php */
		do_action_layoutbridge_adapter( 'admin_print_scripts-widgets.php' );
	}

	/**
	 * Drupal widgets footer.
	 *
	 * Prints Drupal widgets scripts in Elementor editor footer.
	 *
	...
	public function footer() {
		...
		do_action_layoutbridge_adapter( 'admin_footer-widgets.php' );
	}
}
