...
/**
 * @file
 * Contains \Drupal\drupal_layoutbuilder\Controller\DrupalLayoutbuilderController.
 */

namespace Drupal\drupal_layoutbuilder\Controller;
...
use Drupal\drupal_layoutbuilder\DrupalLayoutbuilderPageExporter;
use Drupal\node\Entity\Node;
use Drupal\drupal_layoutbuilder\DrupalLayoutbuilderTemplateController;
use Drupal\image\Entity\ImageStyle;
use Drupal\views\Views;
use Drupal\webform\Entity\Webform;
use Drupal\webform\Utility\WebformOptionsHelper;

class DrupalLayoutbuilderController extends ControllerBase implements ContainerInjectionInterface
{
    ...
    public function update(Request $request)
    {
        ...
        $return_data = DrupalLayoutbuilderPlugin::$instance->update($nid, $request);
        ...
        $deploy_raw = $actions_obj?->save_builder?->data?->settings?->deploy ?? NULL;
        $deploy_enabled = ($deploy_raw === 'yes' || $deploy_raw === TRUE || $deploy_raw === 1 || $deploy_raw === '1');
        if ($deploy_enabled) {
            ...
            if ($node && !$node->isPublished()) {
                ...
                // json_decode(..., false) yields stdClass; export() requires a nested array.
                $raw_layout = $actions_obj->save_builder->data ?? new \stdClass();
                $layoutbuilder_page_data = json_decode(json_encode($raw_layout), true);
                if (!is_array($layoutbuilder_page_data)) {
                    $layoutbuilder_page_data = [];
                }
                $exporter = DrupalLayoutbuilderPageExporter::export($nid, $layoutbuilder_page_data);
    ...
    private function trackWebformUsage($nid, $elements)
    {
        try {
            ...
            \Drupal::logger('drupal_layoutbuilder')->error('Error tracking webform usage: @msg', ['@msg' => $e->getMessage()]);
        }
    }
    ...
    public function editor(Request $request)
    {
        ...
        $editor_data = DrupalLayoutbuilderPlugin::$instance->editor($id);

        // $template = $this->twig->loadTemplate(\Drupal::service('extension.list.module')->getPath('drupal_layoutbuilder') . '/templates/drupal-layoutbuilder-editor.html.twig');
        $template = $this->twig->load(\Drupal::service('extension.list.module')->getPath('drupal_layoutbuilder') . '/templates/drupal-layoutbuilder-editor.html.twig');
        ...
        $html = $template->render([
            'is_rtl' => $dir == 'rtl',
            'drupal_layoutbuilder_data' => $editor_data,
            'base_path' => base_path() . \Drupal::service('extension.list.module')->getPath('drupal_layoutbuilder'),
        ]);
    ...
    public function upload(Request $request)
    {
        ...
        foreach ($request->files->all() as $key => $file) {
            $files[] = DrupalLayoutbuilderPlugin::$instance->sdk->upload_file($file->getPathName(), $file->getClientOriginalName());
        }
    ...
    {
        $newFile = DrupalLayoutbuilderPlugin::$instance->sdk->delete_file($request->get('fid'));
    ...
    public function data($content_type, Request $request)
    {
        ...
        $service = \Drupal::service('drupal_layoutbuilder.drupal_block_service');
