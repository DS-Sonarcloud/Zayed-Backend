...
window.addEventListener("DOMContentLoaded", function() {
  ...
  xhr.open('POST', '/drupal-layoutbuilder/multifile-upload', true);
...
HTML;
  ...
  public function upload(Request $request)
  {
    ...
    if (empty($_FILES['files']) || !isset($_FILES['files']['name']) || count($_FILES['files']['name']) === 0) {
      \Drupal::logger('drupal_layoutbuilder')->error('No files in upload request: @data', ['@data' => print_r($_FILES, TRUE)]);
