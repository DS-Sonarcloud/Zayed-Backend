...
use Drupal\drupal_layoutbuilder\Import_Images;
use DrupalLayoutbuilder\Api;
use DrupalLayoutbuilder\Plugin;
use DrupalLayoutbuilder\TemplateLibrary\Manager;
...
class Drupal_TemplateLibrary_Manager extends Manager
{
    ...
    public function get_library_data(array $args)
    {
        ...
        $controller = new \Drupal\drupal_layoutbuilder\Controller\DrupalLayoutbuilderTemplateController();
        $res = $controller->getLibraryData();
        return $res;
        // die;
        // if (empty($args['sync'])) {
        //     $library_data = DrupalLayoutbuilderPlugin::$instance->sdk->get_remote_templates('remote');
        // }
        // if (!$library_data) {
        //     $library_data = Api::get_library_data(!empty($args['sync']));
        //     DrupalLayoutbuilderPlugin::$instance->sdk->save_remote_templates('remote', $library_data);
        // }
        // $library_data = Api::get_library_data(!empty($args['sync']));
        // return [
    ...
    public function save_template(array $args)
    {
        ...
        if (is_wp_error_layoutbridge_adapter($validate_args)) {
    ...
    public function update_template(array $template_data)
    {
        ...
        if (is_wp_error_layoutbridge_adapter($update)) {
            return $update;
        }
    ...
    public function update_templates(array $args)
    {
        foreach ($args['templates'] as $template_data) {
            ...
            if (is_wp_error_layoutbridge_adapter($result)) {
                return $result;
            }
        }
    ...
    public function get_template_data(array $args)
    {
        ...
        do_action_layoutbridge_adapter('elementor/template-library/before_get_source_data', $args, $source);

        $data = $source->get_data($args);

        do_action_layoutbridge_adapter('elementor/template-library/after_get_source_data', $args, $source);
    ...
    {
        wp_redirect_layoutbridge_adapter(admin_url_layoutbridge_adapter('edit.php?post_type=' . Source_Local::CPT));
    }
     ...
     * @access public
     *
     * @param \WP_Error $error Drupal error instance.
     */
     ...
     *
     * Kill Drupal execution and display HTML error messages after template
     * export was failed.
     *
     * @since 1.0.0
     * @access public
    ...
    private function handle_ajax_request($ajax_request)
    {
        ...
        if (!empty($_REQUEST['editor_post_id'])) {
            $editor_post_id = absint_layoutbridge_adapter($_REQUEST['editor_post_id']);

            if (!get_post_layoutbridge_adapter($editor_post_id)) {
                wp_send_json_error_layoutbridge_adapter(___layoutbridge_adapter('Post not found.', 'elementor'));
            }
        ...
        if (is_wp_error_layoutbridge_adapter($result)) {
            if ('ajax' === $request_type) {
                wp_send_json_error_layoutbridge_adapter($result);
            }
        ...
        if ('ajax' === $request_type) {
            return wp_send_json_success_layoutbridge_adapter($result);
        }
    ...
    private function init_ajax_calls()
    {
        ...
        foreach ($allowed_ajax_requests as $ajax_request) {
            remove_action_layoutbridge_adapter('wp_ajax_elementor_' . $ajax_request);
            add_action_layoutbridge_adapter('wp_ajax_elementor_' . $ajax_request, function () use ($ajax_request) {
