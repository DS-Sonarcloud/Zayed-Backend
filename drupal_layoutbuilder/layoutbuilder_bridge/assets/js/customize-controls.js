...
(function( exports, $ ){
		...
		get: function( args ) {
			...
			if ( params.sort ) {
				...
				notifications.sort( function( a, b ) {
					...
					return collection._addedOrder[ b.code ] - collection._addedOrder[ a.code ]; 
				});
			}
	...
	api.Panel = Container.extend({
		...
		embed: function () {
			var panel = this,
				...
				parentContainer = $( '.customize-pane-parent' ); 
	...
	$( function() {
		...
		(function( state ) {
			...
			selectedChangesetStatus.link( changesetStatus ); 
			saved( true );
			if ( '' === changesetStatus() ) { 
				api.each( function( setting ) {
					if ( setting._dirty ) {
		...
		(function() {
			...
			if ( ! api.settings.changeset.branching && ( ! api.settings.changeset.status || 'auto-draft' === api.settings.changeset.status ) ) {
				removedQueryParams.push( 'changeset_uuid' ); 
			}
		...
		api( 'show_on_front', 'page_on_front', 'page_for_posts', function( showOnFront, pageOnFront, pageForPosts ) {
			...
			handleChange.call( showOnFront, showOnFront() ); 
		...
		function startAutosaving() {
			...
			api.unbind( 'change', startAutosaving ); 
