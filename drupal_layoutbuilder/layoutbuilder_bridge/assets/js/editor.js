		...
		function switchEditor( id, mode ) {
			...
			if ( 'tmce' === mode || 'tinymce' === mode ) {
				...
				if ( editor ) {
					...
					if ( init && window.tinyMCEPreInit.baseURL && ! init.baseURL ) {
						init.baseURL = window.tinyMCEPreInit.baseURL;
					}
					if ( init && window.tinyMCEPreInit.suffix !== undefined && init.suffix === undefined ) {
						init.suffix = window.tinyMCEPreInit.suffix;
					}
					tinymce.init( init );
				}
				...
				if ( editor ) {
					...
					// Sync TinyMCE iframe → textarea before hiding; otherwise the Text tab
					// stays empty while the canvas preview still shows content (Elementor panel).
