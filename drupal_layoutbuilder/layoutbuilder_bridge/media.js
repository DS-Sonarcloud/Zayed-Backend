var ControlMediaItemView = elementor.modules.controls.Media.extend({
  ...
  select: function(e) {
    ...
    fetch(base_url + '/drupal-layoutbuilder/upload', {
      method: 'POST',
  ...
  deleteImage: function(event) {
    ...
    fetch(base_url + '/drupal-layoutbuilder/delete_upload/' + this.getControlValue().id, {
      method: 'DELETE',
    }).then(res=>res.json()).then(data=>{
      this.setValue({
        url: '',
...
var ControlMediaItemViewGallery = elementor.modules.controls.Gallery.extend({
  applySavedValue: function() {
    ...
    this.$el
      .toggleClass('drupal_layoutbuilder-gallery-has-images', hasImages)
      .toggleClass('drupal_layoutbuilder-gallery-empty', !hasImages);
    ...
    images.forEach(function(image) {
      var $thumbnail = jQuery('<div>', {
        'class': 'drupal_layoutbuilder-control-gallery-thumbnail',
        'data-id': image.id
      });
  ...
  select: function(e) {
    ...
    fetch('/drupal-layoutbuilder/upload', {
      method: 'POST',
      enctype: 'multipart/form-data',
      body: formData,
    })
  ...
  resetGallery: function() {
    Promise.all(
      (this.getControlValue() || []).map(function(image) {
        return fetch('/drupal-layoutbuilder/delete_upload/' + image.id, {
          method: 'DELETE',
        }).catch(error => console.log(error));
      })
    ).then(() => {
  ...
  onGalleryThumbnailsClick: function(e) {
    ...
    if ($target.hasClass('drupal_layoutbuilder-control-gallery-thumbnail-remove')) {
      var id = $target.closest('.drupal_layoutbuilder-control-gallery-thumbnail').data('id');
  ...
  removeImageById: function(id) {
    ...
    fetch('/drupal-layoutbuilder/delete_upload/' + id, {
      method: 'DELETE',
    }).catch(error => console.log(error));
