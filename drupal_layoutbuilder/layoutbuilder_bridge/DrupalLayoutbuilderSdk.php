...
/**
 * @file
 * Contains \Drupal\drupal_layoutbuilder\DrupalLayoutbuilderSdk.
 */
...
class DrupalLayoutbuilderSdk
{
  ...
  {
    if (!$this->connection->schema()->tableExists('drupal_layoutbuilder_data')) {
      return null;
    }
    $result = $this->connection->query(
      'SELECT data FROM {drupal_layoutbuilder_data} WHERE uid = :uid ORDER BY id DESC LIMIT 1',
      [':uid' => $uid]
    )->fetch();
  ...
  public function set_data($uid, $data)
  {
    ...
    DrupalLayoutbuilderPageExporter::updateImageUrlsRecursively($data['elements']);

    $this->connection->insert('drupal_layoutbuilder_data')
      ->fields([
        'uid' => $uid,
        'author' => 'admin',
        'timestamp' => time(),
    ...
    $result_count = $this->connection->query("SELECT COUNT(uid) as num FROM drupal_layoutbuilder_data WHERE uid = " . $uid)
      ->fetch();
    $count = $result_count->num - 10;
    if ($count > 0) {
      $result = $this->connection->query("DELETE FROM drupal_layoutbuilder_data WHERE uid = " . $uid . " LIMIT " . $count)
        ->execute();
    }
    ...
    $node->save();
    \Drupal::logger('drupal_layoutbuilder')->notice('Updated node ID: @nid', ['@nid' => $uid]);
  }
  ...
  {
    return $this->connection->query("DELETE FROM drupal_layoutbuilder_data WHERE id = " . $id)
      ->execute();
  }
  ...
  {
    $result = $this->connection->query("SELECT * FROM drupal_layoutbuilder_data WHERE uid = " . $uid)
      ->fetchAll();
  ...
  {
    $result = $this->connection->query("SELECT id FROM drupal_layoutbuilder_data WHERE uid = " . $uid)
      ->fetchAll();
  ...
  {
    $result = $this->connection->query("SELECT * FROM drupal_layoutbuilder_data WHERE id = " . $id)
      ->fetch();
    return json_decode($result->data, true);
  }
  ...
  {
    return $this->connection->query("SELECT id FROM drupal_layoutbuilder_template WHERE type = '" . $type . "'")
      ->fetchAll();
  }
  ...
  {
    $result = $this->connection->query("SELECT * FROM drupal_layoutbuilder_template WHERE id = " . $id)
      ->fetch();
  ...
  {
    $timestamp = time();
    return $this->connection->insert('drupal_layoutbuilder_template')
      ->fields([
        'type' => 'local',
        'name' => !empty($data['title']) ? $data['title'] : ___layoutbridge_adapter('(no title)', 'elementor'),
  ...
  {
    return $this->connection->query("DELETE FROM drupal_layoutbuilder_template WHERE id = " . $id)
      ->execute();
  }
  ...
  {
    return \Drupal::configFactory()->getEditable('drupal_layoutbuilder.template')
  ...
  {
    $data = \Drupal::config('drupal_layoutbuilder.tmps')->get($type);
  ...
  public function init_widgets()
  {
    ...
    foreach ($build_widgets_filename as $widget_filename) {
      // dump(\Drupal::service('extension.list.module')->getPath('drupal_layoutbuilder'). '/layoutbuilder_bridge/widgets/' . $widget_filename . '.php');exit;
      include(\Drupal::service('extension.list.module')->getPath('drupal_layoutbuilder') . '/layoutbuilder_bridge/widgets/' . $widget_filename . '.php');
