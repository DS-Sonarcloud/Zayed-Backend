...
class GrapesJSEditor extends EditorBase implements ContainerFactoryPluginInterface
{
  ...
  public function getJSSettings(Editor $editor)
  {
    ...
    // Aggressively force these plugins to be enabled for the Email Editor experience
    $mandatory_plugins = ['drupal_blocks', 'drupal_asset', 'drupal_storage', 'drupal_fields', 'drupal_basic_blocks', 'drupal_reusable_blocks', 'drupal_unsubscribe', 'drupal_document'];

    // Get the settings for all enabled plugins.
    $enabled_plugins = array_keys($this->pluginManager->getEnabledPlugins($editor));
    $all_plugins_to_process = array_unique(array_merge($enabled_plugins, $mandatory_plugins));

    foreach ($all_plugins_to_process as $plugin_id) {
