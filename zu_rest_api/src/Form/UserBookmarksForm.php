...
/**
 * Displays a Claro-styled table of users who bookmarked a specific node.
 */
class UserBookmarksForm extends FormBase
{

  public function getFormId()
  {
    return 'node_bookmarks_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $nid = NULL)
  {
    ...
    if (empty($flaggings)) {
      $form['empty'] = [
        '#markup' =>
        '<div class="messages messages--warning">No users have bookmarked this content yet.</div>',
      ];
      return $form;
    }
    ...
    foreach ($flaggings as $flagging) {
      ...
      $rows[] = [
        ...
        'link_url' => $user->toUrl('canonical', ['absolute' => TRUE])->toString(),
      ];
    ...
    usort($rows, function ($a, $b) use ($sort_key, $sort_dir) {
      $aVal = strtolower($a[$sort_key]);
      $bVal = strtolower($b[$sort_key]);
      ...
      $table_rows[] = [
        ...
        'username' => [
          'data' => [
            '#type' => 'link',
            '#title' => $row['username'],
            '#url' => \Drupal\Core\Url::fromUri($row['link_url']),
    ...
    $form['download_csv'] = [
      ...
      '#url' => \Drupal\Core\Url::fromRoute('zu_rest_api.user_bookmarks_csv', ['nid' => $nid]),
      '#attributes' => [
  ...
  private function sortableHeader($label, $key, $active, $dir)
  {
    ...
    return [
      'data' => \Drupal\Core\Render\Markup::create(
