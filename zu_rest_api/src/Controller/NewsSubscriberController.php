...
/**
 * Handles News Subscribers using public_user entity + Flag module.
 */
class NewsSubscriberController extends ControllerBase
{
  ...
  /**
   * Subscribe a public user to news updates.
   */
  public function addSubscriber(Request $request): JsonResponse
  {
    ...
    $flag = $flagService->getFlagById('news_subscribe');

    if (!$flag) {
      return new JsonResponse([
        'status' => 'error',
        'message' => 'Flag "news_subscribe" not found.',
      ], 500);
    }
    ...
    if (!$flagging) {
      ...
      $message = "News subscription added successfully.";
    } else {
  ...
  protected function sendSubscriptionEmail(PublicUser $user, ?string $temp_pass = NULL)
  {
    ...
    \Drupal::service('plugin.manager.mail')
      ->mail('zu_rest_api', 'news_subscribe_confirmation', $email, $langcode, $params);
  }
