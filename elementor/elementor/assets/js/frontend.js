...
(function ($) {
  ...
  function initTabsInScope($scope) {
    ...
    $scope.find('.elementor-tabs').each(function () {
      var $tabs = $(this);

      $tabs.off('click.myTabs').on('click.myTabs', '.elementor-tab-title', function (e) {
        e.preventDefault();
        var $title = $(this);
        ...
        $tabs.find('.elementor-tab-title[data-tab="' + id + '"]').addClass('elementor-active');

        var $contents = $tabs.find('> .elementor-tabs-content-wrapper .elementor-tab-content');
        if (!$contents.length) $contents = $tabs.find('.elementor-tab-content');
        $contents.hide();
        $contents.filter('[data-tab="' + id + '"]').show();
      });

      var $titles = $tabs.find('.elementor-tab-title');
      var $contents = $tabs.find('> .elementor-tabs-content-wrapper .elementor-tab-content');
      if (!$contents.length) $contents = $tabs.find('.elementor-tab-content');

      if (!$titles.length || !$contents.length) return;

      var $active = $titles.filter('.elementor-active').first();
      if (!$active.length) {

        var $desktop = $tabs.find('> .elementor-tabs-wrapper .elementor-tab-desktop-title').first();
  ...
  function initAccordionsInScope($scope) {
    ...
    $scope.find('.elementor-accordion').each(function () {
      var $container = $(this);

      $container.off('click.myAccordion').on('click.myAccordion', '.elementor-accordion-item > .elementor-tab-title', function (e) {
        e.preventDefault();
        var $title = $(this);
        ...
        $container.find('.elementor-accordion-item').not($item).each(function () {
          var $it = $(this);
          $it.removeClass('elementor-active').removeClass('active');
          $it.find('.elementor-tab-title').removeClass('elementor-active');
          $it.find('.elementor-tab-content').stop(true, true).slideUp(180);
          $it.find('.elementor-accordion-icon-closed').show();
          $it.find('.elementor-accordion-icon-opened').hide();
        });

        var isActive = $item.hasClass('elementor-active') || $item.hasClass('active');
        if (isActive) {
          $item.removeClass('elementor-active active');
          $title.removeClass('elementor-active');
          $content.stop(true, true).slideUp(180);
          $iconClosed.show();
          $iconOpened.hide();
        } else {
          $item.addClass('elementor-active active');
      ...
      $container.find('.elementor-accordion-item').each(function () {
        var $it = $(this);
        var $content = $it.find('.elementor-tab-content').first();
        var $closed = $it.find('.elementor-accordion-icon-closed');
        var $opened = $it.find('.elementor-accordion-icon-opened');
        if ($it.hasClass('elementor-active') || $it.hasClass('active')) {
    ...
    $scope.find('.elementor-toggle').each(function () {
      var $container = $(this);

      $container.off('click.myToggle').on('click.myToggle', '.elementor-toggle-item > .elementor-tab-title', function (e) {
        e.preventDefault();
        var $title = $(this);
        var $item = $title.closest('.elementor-toggle-item');
        var $content = $item.find('.elementor-tab-content').first();
        var $closed = $item.find('.elementor-toggle-icon-closed');
        var $opened = $item.find('.elementor-toggle-icon-opened');
      ...
      $container.find('.elementor-toggle-item').each(function () {
        var $it = $(this);
        var $content = $it.find('.elementor-tab-content').first();
        var $closed = $it.find('.elementor-toggle-icon-closed');
        var $opened = $it.find('.elementor-toggle-icon-opened');
        if ($it.hasClass('elementor-active') || $it.hasClass('active')) {
          $content.show(); $closed.hide(); $opened.show();
        } else {
          $content.hide(); $closed.show(); $opened.hide();
    ...
    var observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (m) {
        $(m.addedNodes).each(function () {
          var $n = $(this);
          if ($n.is('.elementor-tabs') || $n.find('.elementor-tabs').length) {
            initTabsInScope($n);
          }
          if ($n.is('.elementor-accordion, .elementor-toggle') || $n.find('.elementor-accordion, .elementor-toggle').length) {
...
(function ($) {
  'use strict';
    $(document).on('click', '.elementor-alert-dismiss', function () {
        $(this).parent().fadeOut();
    });
})(jQuery);
...
(function($){
    ...
    function initCounters(root) {
        $(root).find('.elementor-counter-number').each(function(){
            observeCounter($(this));
        });
    }
